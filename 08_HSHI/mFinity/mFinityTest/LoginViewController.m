...
- (void)viewDidLoad {
    ...
    if (bgImage==nil) {
        imageView.image = [UIImage imageNamed:@"lightblue_port_login.png"];
    }else{
    ...
    initPwd.tintColor = [UIColor whiteColor];
    [initPwd setImageEdgeInsets:UIEdgeInsetsMake(0.0, 0.0, 0.0, 5.0)];
    //[initPwd setTitleEdgeInsets:UIEdgeInsetsMake(0.0, 10.0, 0.0, 0.0)];
...
//    dvcid = [MFinityAppDelegate getUUID];
//    NSLog(@"### 로그인 UDID : %@", dvcid);
...
- (IBAction)initPwdButton:(id)sender {
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    appDelegate.naviBarColor = @"#2D4260";
    ...
    appDelegate.target_url = [[NSString alloc] initWithFormat:@"https://im.hshi.co.kr/pages/pw/InitPass_SmartOn.aspx"];
...
//                paramString = [[NSString alloc]initWithFormat:@"id=%@&pwd=%@&dvcid=%@&dvcgubn=P&tel_corp=%@&os_ver=%@&extra_ram=%@&extra_total_volume=%@&extra_usable_volume=%@&usable_volume=%@&push_1=%@&push_2=-&rooting=%@&dvcOS=%@&RES_VER=%@&MF_VER=%@&returnType=JSON",encodingID, encodingPWD, dvcid,carrierName,osVersion,extra_ram,extra_total_volume,extra_usable_volume,usable_volume,appDelegate.appDeviceToken,isRooting,osName,RES_VER,MF_VER];
            ...
            [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //로그인호출",__func__] result:nil];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    if ([methodName isEqualToString:@"getOfflineMenuList"]) {
        ...
        NSLog(@"[MFinityAppDelegate getAllValueUrlDecoding:dataDic] : %@",[MFinityAppDelegate getAllValueUrlDecoding:dataDic]);
        
//        self.devId = dvcid;
//        NSLog(@"로그인웹서비스 호출 결과 self.devId : %@", self.devId);
        
        self.loginType = @"LOGIN";
        NSString *result = [super resultUserCheck:[MFinityAppDelegate getAllValueUrlDecoding:dataDic]];
        [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //리턴처리완료",__func__] result:result];
       ...
       //[self callGetGroupImage];
        ...
        if ([result isEqualToString:@"SUCCEED"]) {
            ...
            if([prefs objectForKey:@"FCM_TOKEN"]==nil || ![appDelegate.fcmToken isEqualToString:[prefs objectForKey:@"FCM_TOKEN"]]){
                ...
                NSString *paramString = [[NSString alloc]initWithFormat:@"cuserno=%@&dvcid=%@&pushid1=%@&pushid2=-",appDelegate.user_no, [prefs objectForKey:@"UUID"], appDelegate.fcmToken];
                NSData *paramData = [paramString dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
                
                [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //FCM 키가 없거나 다를 경우 PUSHID_UPDATE 호출",__func__] result:result];
...
//                NSString *newDvcid = [MFinityAppDelegate getUUID];
//                NSLog(@"로그인성공. DEVICE_ID 값 없을경우 새로 생성한 기기 : %@", newDvcid);
//                NSLog(@"로그인성공. DEVICE_ID 값 없을경우 기존에 생성된 기기 : %@", dvcid);
                
                [prefs setObject:[prefs objectForKey:@"UUID"] forKey:@"DEVICE_ID"];
                [prefs synchronize];
                
                [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //기기ID가 프리퍼런스에 없을 경우 저장",__func__] result:result];
            }
