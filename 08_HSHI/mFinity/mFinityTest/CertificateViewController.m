...
//                 isTemperature, isGPS, isCamera, isFrontCamera, isVoip, isTelephony, totalVolume, dvcMdlCd];
    ...
    [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //인증호출",__func__] result:nil];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    if ([methodName isEqualToString:@"getOfflineMenuList"]) {
        ...
        //self.devId = dvcid;
//        NSLog(@"인증 웹서비스 호출 결과 self.devId : %@", self.devId);
        ...
        if ([result isEqualToString:@"SUCCEED"]) {
            ...
            if([prefs objectForKey:@"FCM_TOKEN"]==nil || ![appDelegate.fcmToken isEqualToString:[prefs objectForKey:@"FCM_TOKEN"]]){
                ...
                [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //리턴처리완료/FCM 키가 없거나 다를 경우 PUSHID_UPDATE 호출",__func__] result:result];
...
//                NSLog(@"인증성공. DEVICE_ID 값 없을경우 새로 생성한 기기 : %@", newDvcid);
//                NSLog(@"인증성공. DEVICE_ID 값 없을경우 기존에 생성된 기기  : %@", dvcid);
                
                [prefs setObject:[prefs objectForKey:@"UUID"] forKey:@"DEVICE_ID"];
                [prefs synchronize];
                
                [appDelegate loginHistoryToLogFile:[NSString stringWithFormat:@"%s //리턴처리완료/기기ID가 프리퍼런스에 없을 경우 저장",__func__] result:result];
            }
