...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if ([alertView.title isEqualToString:NSLocalizedString(@"message66", @"")]) {
        ...
        if(buttonIndex==0){
            ...
            }else if([deleteTitle isEqualToString:NSLocalizedString(@"message40", @"")]){ //사진삭제
                NSArray *arrayPaths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
                NSString *docDir = [arrayPaths objectAtIndex:0];
            ...
            }else if([deleteTitle isEqualToString:NSLocalizedString(@"message41", @"")]){ //동영상삭제
                NSArray *arrayPaths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
                NSString *docDir = [arrayPaths objectAtIndex:0];
            ...
            }else if([deleteTitle isEqualToString:NSLocalizedString(@"message116", @"")]){ //웹앱삭제
                MFinityAppDelegate *appDelegate = (MFinityAppDelegate *)[[UIApplication sharedApplication] delegate];
