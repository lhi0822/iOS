...
- (void)viewDidLoad
{
    ...
    if (screenHeight/screenWidth <= 1.5) {
        ...
        [hostField setFrame:CGRectMake(hostField.frame.origin.x
                                       , hostField.frame.origin.y+SIZEGAP,
                                       hostField.frame.size.width,
                                       hostField.frame.size.height)];
        [portField setFrame:CGRectMake(portField.frame.origin.x
                                       , portField.frame.origin.y+SIZEGAP,
                                       portField.frame.size.width,
                                       portField.frame.size.height)];
        [testButton setFrame:CGRectMake(testButton.frame.origin.x
                                        , testButton.frame.origin.y+SIZEGAP,
                                        testButton.frame.size.width,
                                        testButton.frame.size.height)];
        ...
        [guideLabel setFrame:CGRectMake(guideLabel.frame.origin.x
                                        , guideLabel.frame.origin.y+SIZEGAP,
                                        guideLabel.frame.size.width,
                                        guideLabel.frame.size.height)];
    }
    ...
    if (bgImage==nil) {
        _imageView.image = [UIImage imageNamed:@"lightblue_port_login.png"];
    }else{
        _imageView.image = bgImage;
    }
    [testButton setTitle:NSLocalizedString(@"SERVER CONNECT TEST", @"server connect test") forState:UIControlStateNormal];
    ...
    testButton.backgroundColor = [appDelegate myRGBfromHex:@"0093d5"];
    testButton.layer.cornerRadius = testButton.frame.size.width/30;
...
-(IBAction)TestConnection{
    ...
    if (!isButton) {
        ...
        if ([portField.text isEqualToString:@""]) {
            NSArray *tmpArr = [mainUrl componentsSeparatedByString:@":"];
            mainUrl = [NSString stringWithFormat:@"%@:%@",[tmpArr objectAtIndex:0],[tmpArr objectAtIndex:1]];
        }
