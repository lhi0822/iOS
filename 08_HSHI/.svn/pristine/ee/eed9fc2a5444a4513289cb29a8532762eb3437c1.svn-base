    ...
    if([appDelegate.mainType isEqualToString:@"1"]){
        if([appDelegate.tabBarType isEqualToString:@"B"]){
            ...
            if([appDelegate isIphoneX]){
                //[myTableView setFrame:CGRectMake(myTableView.frame.origin.x, appDelegate.scrollView.frame.size.height, myTableView.frame.size.width, myTableView.frame.size.height-self.navigationController.navigationBar.frame.size.height)];
                [myTableView setFrame:CGRectMake(myTableView.frame.origin.x, appDelegate.scrollView.frame.size.height, myTableView.frame.size.width, viewHeight-(appDelegate.scrollView.frame.origin.y+appDelegate.scrollView.frame.size.height))];
            } else {
                [myTableView setFrame:CGRectMake(myTableView.frame.origin.x, appDelegate.scrollView.frame.size.height, myTableView.frame.size.width, viewHeight-(appDelegate.scrollView.frame.origin.y+appDelegate.scrollView.frame.size.height))];
            }
        ...
        if([appDelegate.tabBarType isEqualToString:@"B"]){
            [imageView setFrame:CGRectMake(myTableView.frame.origin.x, myTableView.frame.origin.y, myTableView.frame.size.width, appDelegate.scrollView.frame.origin.y-self.navigationController.navigationBar.frame.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)];
            [myTableView setFrame:CGRectMake(myTableView.frame.origin.x, myTableView.frame.origin.y, myTableView.frame.size.width, appDelegate.scrollView.frame.origin.y-self.navigationController.navigationBar.frame.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)];
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Save" style:UIBarButtonItemStylePlain target:self action:@selector(rightBtnClick)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UISwitch *mySwitch = [[UISwitch alloc] initWithFrame:CGRectMake(mainWidth-65, 9, 94, 31)];
    ...
    if ([[menuCheckDictionary objectForKey:[NSString stringWithFormat:@"%ld",(long)indexPath.row]] isEqualToString:@"TRUE"]) {
		[mySwitch setOn:YES];
	} else {
		[mySwitch setOn:NO];
	}
    ...
    cell.txtTitle.text = [menuNameDictionary objectForKey:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
