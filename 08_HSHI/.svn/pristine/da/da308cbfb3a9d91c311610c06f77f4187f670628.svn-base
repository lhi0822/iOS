...
//    appDelegate.mainType = @"3";
//    appDelegate.tabBarType = @"T";
    ...
    if([appDelegate.mainType isEqualToString:@"1"]){
        ...
        if([appDelegate.tabBarType isEqualToString:@"B"]){
            ...
            [imageView setFrame:CGRectMake(mainScrollView.frame.origin.x, mainScrollView.frame.origin.y, mainScrollView.frame.size.width, imageView.frame.size.height)];
        ...
        if ([[[menuArray objectAtIndex:0]objectForKey:@"V0"] isEqualToString:@"False"]) {
            ...
            if ([[[menuArray objectAtIndex:1]objectForKey:@"V8_1"]isEqualToString:@"#"] ) {
                ...
                if ([appDelegate.mainType isEqualToString:@"1"]) {
                    ...
                    if([appDelegate.tabBarType isEqualToString:@"B"]) [self defaultMenuSetting];
                    else [self menuSetting];
...
-(void)verticalMenuSetting{
    NSLog(@"verticalMenuSetting imgViewSize : %f", imageView.frame.size.height);
    ...
    NSLog(@"platformNumber : %@", platformNumber);
    ...
    if ([[[arr objectAtIndex:arr.count-1] lowercaseString] isEqualToString:@"plus"]||[[[arr objectAtIndex:arr.count-1] lowercaseString] isEqualToString:@"max"]){
        isPlusDevice = YES;
    }else{
        isPlusDevice = NO;
    }
    ...
    if (colCount == 3) {
        ...
        //2018.06 UI개선
        if([appDelegate.tabBarType isEqualToString:@"B"]){
            //[mainScrollView setFrame:CGRectMake(mainScrollView.frame.origin.x, mainScrollView.frame.origin.y, mainScrollView.frame.size.width, imageView.frame.size.height)];
            [mainScrollView setFrame:CGRectMake(mainScrollView.frame.origin.x, mainScrollView.frame.origin.y, mainScrollView.frame.size.width, appDelegate.scrollView.frame.origin.y-self.navigationController.navigationBar.frame.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)];
        } else if([appDelegate.tabBarType isEqualToString:@"T"]){
            [mainScrollView setFrame:CGRectMake(mainScrollView.frame.origin.x, appDelegate.scrollView.frame.size.height, mainScrollView.frame.size.width, mainScrollView.frame.size.height)];
        }
        mainScrollView.contentSize = CGSizeMake(mainScrollView.frame.size.width, (ICON_START_VERTICAL+ICON_HEIGHT+TITLE_HEIGHT+5)*result);
...
-(void) buttonTouched:(id)sender {
	//메뉴아이콘이 터치 되었을때
	int indexBtn = (int)[sender tag] - (int)MENU_ICON_TAG;
...
- (void)menuHandler{
    ...
    if ([menuKind isEqualToString:@"M"]) {
        ...
        if ([menuType isEqualToString:@"B1"]) {
            ...
            NSLog(@"main target_url : %@",appDelegate.target_url);
            NSLog(@"main url : %@", url);
            ...
            @try {
                if ([nativeAppURL isEqualToString:@"#"]) {
                    ...
                    if ([pref objectForKey:appDelegate.menu_no]==nil) {
                        ...
                        /*
                        nativeAppURL : itms-services://?action=download-manifest&url=https://roms.dbvalley.com/mfinity/upload/nativeapp/10/1/korchampass_1.1.6-002.plist
                        url : korchampass://?
                        설치됨
                        stringURLScheme : com.dbvalley.mfinity.emmp
                        callUrl : korchampass://?://?call=com.dbvalley.mfinity.emmp
                        */
...
//                            [[UIApplication sharedApplication]openURL:[NSURL URLWithString:appDelegate.target_url]];
//
//                        } else {
//                            NSLog(@"설치안됨");
