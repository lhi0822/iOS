...
- (void)viewDidLoad {
    ...
    [_previewView setFrame:CGRectMake(0, 0, viewWidth, viewHeight)];
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"닫기" style:UIBarButtonItemStylePlain target:self action:@selector(close)];
    [self.navigationController.navigationBar setTintColor:[UIColor whiteColor]];
    
    int radius = _previewView.frame.size.width;
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, _previewView.bounds.size.width, _previewView.bounds.size.height) cornerRadius:0];
    UIBezierPath *squarePath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(_previewView.center.x-(radius/4*3/2), _previewView.center.y-(radius/4*3/2), radius/4*3, radius/4*3) cornerRadius:0];
    [path appendPath:squarePath];
    [path setUsesEvenOddFillRule:YES];
    
//    전체사각형
//    CAShapeLayer* guidePath = [CAShapeLayer layer];
//    guidePath.path = path.CGPath;
//    [guidePath setFillColor:[UIColor clearColor].CGColor];
//    [guidePath setStrokeColor:[UIColor blueColor].CGColor];
//    guidePath.frame=CGRectMake(0, 0,100,30);
//    [_previewView.layer addSublayer:guidePath];
    
//  배경 어둡게
    CAShapeLayer *bgFillLayer = [CAShapeLayer layer];
    bgFillLayer.path = path.CGPath;
    bgFillLayer.fillRule = kCAFillRuleEvenOdd;
    bgFillLayer.fillColor = [UIColor blackColor].CGColor;
    bgFillLayer.opacity = 0.5;
    [_previewView.layer addSublayer:bgFillLayer];
    
//  꺽쇠
    CGFloat pointX = _previewView.center.x-(radius/4*3/2);
    CGFloat pointY = _previewView.center.y-(radius/4*3/2);
    UIBezierPath* guidePath = [UIBezierPath bezierPath];
    
//    ⎡
    [guidePath moveToPoint:CGPointMake(pointX+30, pointY)];
    [guidePath addLineToPoint:CGPointMake(pointX, pointY)];
    [guidePath addLineToPoint:CGPointMake(pointX, pointY+30)];
//    ⎤
    [guidePath moveToPoint:CGPointMake((pointX+radius/4*3)-30, pointY)];
    [guidePath addLineToPoint:CGPointMake((pointX+radius/4*3), pointY)];
    [guidePath addLineToPoint:CGPointMake((pointX+radius/4*3), pointY+30)];
//    ⎣
    [guidePath moveToPoint:CGPointMake(pointX+30, pointY+radius/4*3)];
    [guidePath addLineToPoint:CGPointMake(pointX, pointY+radius/4*3)];
    [guidePath addLineToPoint:CGPointMake(pointX, (pointY+radius/4*3)-30)];
//     ⎦
    [guidePath moveToPoint:CGPointMake((pointX+radius/4*3)-30, pointY+radius/4*3)];
    [guidePath addLineToPoint:CGPointMake(pointX+radius/4*3, pointY+radius/4*3)];
    [guidePath addLineToPoint:CGPointMake(pointX+radius/4*3, (pointY+radius/4*3)-30)];
    
    [squarePath appendPath:guidePath];

    CAShapeLayer *guideLayer = [CAShapeLayer layer];
    guideLayer.path = guidePath.CGPath;
    guideLayer.strokeColor = [[appDelegate myRGBfromHex:@"00ff00"] CGColor];
    guideLayer.fillColor = nil;
    guideLayer.lineWidth = 3.0f;
    [_previewView.layer addSublayer:guideLayer];
    
    UIBezierPath *line = [UIBezierPath bezierPath];
    [line moveToPoint:CGPointMake(pointX, _previewView.center.y)];
    [line addLineToPoint:CGPointMake((pointX+radius/4*3), _previewView.center.y)];
    [squarePath appendPath:line];
    
    CAShapeLayer *lineLayer = [CAShapeLayer layer];
    lineLayer.path = line.CGPath;
    lineLayer.strokeColor = [[UIColor redColor] CGColor];
    lineLayer.lineWidth = 1.0f;
    [_previewView.layer addSublayer:lineLayer];
    ...
    [self.scanner startScanningWithResultBlock:^(NSArray *codes) {
        for (AVMetadataMachineReadableCodeObject *code in codes) {
            if (code.stringValue && [self.uniqueCodes indexOfObject:code.stringValue] == NSNotFound) {
                ...
                NSString *accessCode = code.stringValue;
                [self dismissViewControllerAnimated:YES completion:^(void){
                    [self resultBarcode:accessCode];
                }];
            }
        }
    }];
