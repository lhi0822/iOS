...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data{
    ...
    if (error==nil) {
        if([_deleteFlag isEqualToString:@"true"]){
        ...
        if(_uploadFilePathArray.count-1 == progressTag) {
            [self.delegate returnArray:returnArray WithError:error.localizedDescription];
        }
