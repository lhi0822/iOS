...
- (void) setTabBar:(NSDictionary *)dic {
    ...
    _controllers = [NSDictionary dictionaryWithObjectsAndKeys:mainView,@"1",noticeView,@"2",myMenuView,@"3",settingView,@"4",exit,@"5",webView1,@"6",webView2,@"7",webView3,@"8",webView4,@"9",webView5,@"10",webView6,@"11", nil];
    ...
    for (int i=0; i<[dic count]; i++) {
        ...
        [setTabs addObject:[_controllers objectForKey:tabVar]];
...
-(void)tabBarController:(UITabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController{
    ...
    UINavigationController *exit = [_controllers objectForKey:@"5"];
    UINavigationController *web1 = [_controllers objectForKey:@"6"];
    UINavigationController *web2 = [_controllers objectForKey:@"7"];
    UINavigationController *web3 = [_controllers objectForKey:@"8"];
    UINavigationController *web4 = [_controllers objectForKey:@"9"];
    UINavigationController *web5 = [_controllers objectForKey:@"10"];
    UINavigationController *web6 = [_controllers objectForKey:@"11"];
    UINavigationController *main = [_controllers objectForKey:@"1"];
    UINavigationController *mymenu = [_controllers objectForKey:@"3"];
    UINavigationController *notice = [_controllers objectForKey:@"2"];
    UINavigationController *setting = [_controllers objectForKey:@"4"];
    ...
    if (state==UIApplicationStateActive) {
        ...
        if (_isLogin) {
            ...
            if ([type isEqualToString:@"M1"]) {
                ...
                [self removeData];
                alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message152", @"") message:NSLocalizedString(@"message153", @"") delegate:self cancelButtonTitle:NSLocalizedString(@"message51", @"") otherButtonTitles: nil];
                [alertView show];
                
            }else if([type isEqualToString:@"E"]){
                _userInfo = userInfo;
                if (_isLogin) {
                    //alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message121", @"") message:body delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"message51", @""), nil];
        ...
        if ([type isEqualToString:@"M2"]) {
            ...
            _userInfo = userInfo;
            if (_isLogin) {
                //UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message121", @"") message:body delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"message51", @""), nil];
