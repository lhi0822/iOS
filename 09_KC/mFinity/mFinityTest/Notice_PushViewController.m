...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"편집" style:UIBarButtonItemStylePlain target:self action:@selector(deleteButtonClick)];
    ...
    label.text = @"Push"; //appDelegate.noticeTitle;
...
- (void)leftButtonClick{
    if (noticeViewFlag) {
        ...
        if (appDelegate.isAES256) {
            ...
            //dev_id / returnType=JSON / encType=AES256
        }else{
            url = [NSString stringWithFormat:@"%@/getNoticeList2",appDelegate.main_url];
            ...
            if (pPno==0) {
                ...
                for(int j=0; j<pushList.count; j++){
                    ...
                    if(isAllCheck){
                        ...
                        //[self.rowCheckDictionary setObject:@"N" forKey:pushNo];
                        //[self.checkArray removeObject:pushNo];
                        //[self.indexArray removeObject:[NSString stringWithFormat:@"%@", pushNo]];
                    }
                }
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *titleString;
    UILabel *dateLabel = [[UILabel alloc] initWithFrame:CGRectMake(cell.frame.size.width-25, 0, 70, 50)];
    ...
    switch ([prefs integerForKey:@"FONT_SIZE"]) {
        ...
        case 2:
            ...
            [dateLabel setFrame:CGRectMake(cell.frame.size.width-25, 0, 70, 70)];
            break;
        default:
            break;
    }
