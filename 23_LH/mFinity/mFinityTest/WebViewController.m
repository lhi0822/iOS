...
@interface WebViewController ()<MFNewWebViewDelegate>{
    ...
    NSTimer *gpsTimer;
    BOOL isGisGps;
    NSString *gisCallback;
    NSString *gisUserspec;
    
    NSString *gpsCallback;
}
...
- (void)viewDidLoad
{
    ...
    self.webHistories = [[NSMutableArray alloc]init];
...
//    self.navigationItem.rightBarButtonItem = customBarItemRight;
    ...
    if ([appDelegate.menu_title length] > 9 && [data length] > 18) {
        ...
        if (appDelegate.isMainWebView) {
            ...
            _label.textColor = [appDelegate myRGBfromHex:appDelegate.cNaviFontColor]; //[appDelegate myRGBfromHex:appDelegate.naviFontColor];
        }else{
            if ([appDelegate.naviIsShadow isEqualToString:@"Y"]) {
                _label.shadowOffset = CGSizeMake([appDelegate.naviShadowOffset floatValue], [appDelegate.naviShadowOffset floatValue]);
                _label.shadowColor = [appDelegate myRGBfromHex:appDelegate.naviShadowColor];
            }
            _label.textColor = [appDelegate myRGBfromHex:appDelegate.cNaviFontColor]; //_label.textColor = [appDelegate myRGBfromHex:appDelegate.naviFontColor];
        }
    ...
    NSLog(@"appDelegate.menu_title : %@", appDelegate.menu_title);
...
//    [userController addScriptMessageHandler:self name:@"executeImageEditor"]; //js파일에 없음
    [userController addScriptMessageHandler:self name:@"executeGallery"];
    [userController addScriptMessageHandler:self name:@"executeGISGps"];
    ...
    [userController addScriptMessageHandler:self name:@"executeRetrieveToArray"];
...
//    [userController addScriptMessageHandler:self name:@"executeWebBack"]; //js파일에 없음
    [userController addScriptMessageHandler:self name:@"executeWebHistoryClear"];
...
//    [userController addScriptMessageHandler:self name:@"getPreference"]; //js파일에없음
...
//    [userController addScriptMessageHandler:self name:@"setPreference"]; //js파일에없음
//    [userController addScriptMessageHandler:self name:@"setWebHistoryClear"]; //executeWebHistoryClear로 변경
...
- (void)initWKWebView{
    ...
    if ([[page_url substringToIndex:6] isEqualToString:@"http:/"]||[[page_url substringToIndex:7] isEqualToString:@"https:/"]||[page_url hasPrefix:@"hdwebview://"]) {
        ...
        if(appDelegate.target_method==nil) appDelegate.target_method = @"POST";
            ...
            NSURL *nsurl=[NSURL URLWithString:page_url];
//        NSURL *nsurl=[NSURL URLWithString:@"https://roms.dbvalley.com/forcewave/mfnpSample.do"];
//            NSURL *nsurl=[NSURL URLWithString:@"https://roms.dbvalley.com/forcewave"];
//        NSURL *nsurl=[NSURL URLWithString:@"http://211.170.81.197:8090"];
        NSMutableURLRequest *nsrequest = [NSMutableURLRequest requestWithURL:nsurl];
        [nsrequest setHTTPMethod:appDelegate.target_method];
        [nsrequest setHTTPBody:[appDelegate.target_param dataUsingEncoding:NSUTF8StringEncoding]];
...
//    __block UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(screen.size.width-labelSize, [UIApplication sharedApplication].statusBarFrame.size.height, labelSize+10, 44)];
    __block UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, [UIApplication sharedApplication].statusBarFrame.size.height, screenWidth, 44)];
    ...
    titleLabel.backgroundColor = [appDelegate myRGBfromHex:appDelegate.cNaviColor]; //[appDelegate myRGBfromHex:appDelegate.naviBarColor];
    titleLabel.textColor = [appDelegate myRGBfromHex:appDelegate.cNaviFontColor]; //[appDelegate myRGBfromHex:appDelegate.naviFontColor];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.font = [UIFont boldSystemFontOfSize:20];//[UIFont systemFontOfSize:15.0f weight:0.0f];
...
-(void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([mfnpName isEqualToString:@"executeBackKeyEvent"]) {
    ...
    else if ([mfnpName isEqualToString:@"executeGallery"]) {
        isCamera = YES;
        [self getImage:dic];
    }
    else if ([mfnpName isEqualToString:@"executeGISGps"]) {
        NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
        NSString *userSpecific = [dic objectForKey:@"userSpecific"];
        ...
        [self executeGISGps:callBackFunc :userSpecific :mode];
    }
    ...
    else if ([mfnpName isEqualToString:@"executeRetrieveToArray"]) {
        NSString *dbName = [dic objectForKey:@"dbName"];
        NSString *selectStmt = [dic objectForKey:@"selectStmt"];
        NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
        NSString *userSpecific = [dic objectForKey:@"userSpecific"];
        [self executeRetrieveToArray:dbName :selectStmt :callBackFunc :userSpecific];
    }
        ...
        if([dic objectForKey:@"format"]!=nil) format = [dic objectForKey:@"format"];
        if([dic objectForKey:@"mode"]!=nil) mode = [dic objectForKey:@"mode"];
        
        [self executeSignpad:callBackFunc :userSpecific :format :mode];
    }
        ...
        NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
        NSString *userSpecific = [dic objectForKey:@"userSpecific"];
        if ([dic objectForKey:@"specName"]!=nil) {
            [self getDeviceSpec:callBackFunc :userSpecific :[dic objectForKey:@"specName"]];
        }else{
    ...
    else if ([mfnpName isEqualToString:@"executeWebHistoryClear"]) {
        NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
        NSString *userSpecific = [dic objectForKey:@"userSpecific"];
        [self executeWebHistoryClear:callBackFunc :userSpecific];
    }
//    else if ([mfnpName isEqualToString:@"setRemoveWebViewData"]) {
//        NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
//        NSString *userSpecific = [dic objectForKey:@"userSpecific"];
//        BOOL cookie = YES;
//        BOOL cache = YES;
//
//        if([dic objectForKey:@"cookie"]!=nil) cookie = [dic objectForKey:@"cookie"];
//        if([dic objectForKey:@"cache"]!=nil) cache = [dic objectForKey:@"cache"];
//
//        [self setRemoveWebViewData:callBackFunc :userSpecific :cookie :cache];
//    }
...
- (void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation{
    ...
    [self.webHistories addObject:[NSString stringWithFormat:@"%@", webView.URL]];
...
//                [self executeCamera:callBackFunc :userSpecific];
            ...
            }else if([host isEqualToString:@"executeGallery"]){
                NSDictionary *dic = [self getParameters:[url query]];
                NSLog(@"executeGallery dic : %@", dic);
                isCamera = YES;
                [self getImage:dic];
                
            }else if ([host isEqualToString:@"executeGISGps"]) {
                NSDictionary *dic = [self getParameters:[url query]];
                NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
                NSString *userSpecific = [dic objectForKey:@"userSpecific"];
                NSString *mode = [dic objectForKey:@"mode"];
            ...
            }else if([host isEqualToString:@"executeRetrieveToArray"]){
                NSDictionary *dic = [self getParameters:[url query]];
                NSString *dbName = [dic objectForKey:@"dbName"];
                NSString *selectStmt = [dic objectForKey:@"selectStmt"];
                NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
            ...
            }else if([host isEqualToString:@"executeWebHistoryClear"]){
                NSDictionary *dic = [self getParameters:[url query]];
                NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
                NSString *userSpecific = [dic objectForKey:@"userSpecific"];
                [self executeWebHistoryClear:callBackFunc :userSpecific];
                
            }/*else if ([host isEqualToString:@"setRemoveWebViewData"]) {
                NSDictionary *dic = [self getParameters:[url query]];
                NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
                NSString *userSpecific = [dic objectForKey:@"userSpecific"];
                BOOL cookie = [dic objectForKey:@"cookie"];
                BOOL cache = [dic objectForKey:@"cache"];
                [self setRemoveWebViewData:callBackFunc :userSpecific :cookie :cache];
...
- (void)executeFileUploadReturn:(NSNotification *)notification {
    ...
    if(returnArr!=nil){
        ...
        //        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"VALUE"
        //                                                            message:jsonString
        //                                                           delegate:nil
        //                                                  cancelButtonTitle:@"OK"
        //                                                  otherButtonTitles: nil];
...
-(void)executeGISGps:(NSString *)callBackFunc :(NSString *)userSpecific :(NSString *)mode{
    NSString *jsCommand;
    NSError *error;
...
//    _locationManager.distanceFilter = kCLDistanceFilterNone;
    ...
    isGisGps = YES;
    gisCallback = callBackFunc;
    gisUserspec = userSpecific;
    
    if ([self gpsAvailable]) {
        if([mode isEqualToString:@"1"]){
//            gpsTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(gpsLocationTimer:) userInfo:nil repeats:YES];
...
//            [gpsTimer invalidate];
            ...
            NSDictionary *resultDic = [[NSDictionary alloc]initWithObjectsAndKeys:@"OFF", @"STATUS", nil];
            NSData *jsonData = [NSJSONSerialization dataWithJSONObject:resultDic options:NSJSONWritingPrettyPrinted error:&error];
            NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
            jsonString = [jsonString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];
            
            jsCommand = [NSString stringWithFormat:@"%@('%@','%@')",callBackFunc,userSpecific,jsonString];
            NSLog(@"jsCommand : %@", jsCommand);
//            [mywebView stringByEvaluatingJavaScriptFromString:jsCommand];
            [self evaluateJavaScript:jsCommand];
        }
        ...
        NSDictionary *resultDic = [[NSDictionary alloc]initWithObjectsAndKeys:@"ON_FAILED", @"STATUS", nil];
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:resultDic options:NSJSONWritingPrettyPrinted error:&error];
        NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
...
-(void)gpsLocationTimer:(NSTimer *)timer {
    ...
    NSString *altitude = [NSString stringWithFormat:@"%f",_locationManager.location.altitude];
    
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"HH:mm:ss"];
    NSString *timestamp = [dateFormatter stringFromDate:_locationManager.location.timestamp];
    
    NSString *speed = [NSString stringWithFormat:@"%f",_locationManager.location.speed];
    NSString *accuracy = [NSString stringWithFormat:@"%f",_locationManager.desiredAccuracy];
    NSString *bearing = [NSString stringWithFormat:@"%f",_locationManager.location.course];
    
    NSLog(@"latitude : %@", latitude);
    NSLog(@"longitude : %@", longitude);
    NSLog(@"altitude : %@", altitude);
    NSLog(@"timestamp : %@", timestamp);
    NSLog(@"speed : %@", speed);
    NSLog(@"accuracy : %@", accuracy);
    NSLog(@"bearing : %@", bearing);
    
    NSError *error;
    NSDictionary *resultDic = [[NSDictionary alloc]initWithObjectsAndKeys:@"SUCCEED", @"RESULT", latitude, @"LATITUDE", longitude, @"LONGITUDE", altitude, @"ALTITUDE", timestamp, @"TIME", accuracy, @"ACCURACY", bearing, @"BEARING", speed, @"SPEED", nil];
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:resultDic options:NSJSONWritingPrettyPrinted error:&error];
    NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    jsonString = [jsonString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
-(void)executePush:(NSString *)callBackFunc :(NSString *)userSpecific :(NSString *)userList :(NSString *)message{
    ...
    [_callBackDic setValue:[message urlEncodeUsingEncoding:NSUTF8StringEncoding] forKey:@"message"];
...
-(void)executeRetrieveToArray:(NSString *)dbName :(NSString *)selectStmt :(NSString *)callBackFunc :(NSString *)userSpecific{
    NSString *documentPath = [self makeDBFile];
    ...
    if (sqlite3_open([documentPath UTF8String], &database) == SQLITE_OK) {
        ...
        if (sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK) {
            ...
            [dict1 setObject:@"FAILED" forKey:@"RESULT"];
            [dict1 setObject:NSLocalizedString(str, @"") forKey:@"MESSAGE"];
...
-(void)executeSignpad:(NSString *)callBackFunc :(NSString *)userSpecific :(NSString *)format :(NSString *)mode{
    SignPadViewController *vc = [[SignPadViewController alloc]init];
...
-(void) executeVideoPlayer:(NSString *)streamingUrl{ //20181031 호출부분 추가해야함
    ...
    AVPlayerViewController *playView = [AVPlayerViewController new];
    playView.showsPlaybackControls = YES;
    playView.player.actionAtItemEnd = AVPlayerActionAtItemEndPause;
    
    playView.player = [[AVPlayer alloc] initWithURL:movieURL];
...
//    documentFolder = [documentFolder stringByAppendingPathComponent:appDelegate.comp_no];
//    NSString *photoFolder = @"photo";
//    NSString *saveFolder = [documentFolder stringByAppendingFormat:@"/%@/100024(210503-110644).jpg",photoFolder];
//    NSLog(@"saveFolder : %@", saveFolder);
//    ///var/mobile/Containers/Data/Application/E59AFB83-233F-4CA1-B453-06D8D1013704/Documents/90/photo/100024(210503-110644).jpg
//    imagePath = saveFolder;
    
    imagePath = @"https://roms.dbvalley.com/mfnp/img/mfinity_logo.png";
    ...
    @try{
        ...
        if ([imagePath rangeOfString:@"http:/"].location!=NSNotFound||[imagePath rangeOfString:@"https:/"].location!=NSNotFound) {
...
-(void)getDeviceSpec:(NSString *)callBackFunc :(NSString *)userSpecific :(NSString *)specName{
    NSDictionary *deviceSpec = [self getDeviceSpec:specName];
...
-(void)getDeviceSpec:(NSString *)callBackFunc :(NSString *)userSpecific{
    NSString *spec = [self getJsonStringByDictionary:[self getDeviceSpec:nil]];
...
-(NSDictionary *)getDeviceSpec :(NSString *)specName{
    ...
    NSMutableDictionary *returnDic = [[NSMutableDictionary alloc]init];
    if(specName!=nil){
        if([specName isEqualToString:@"MODEL_NAME"]) [returnDic setObject:modelName  forKey:@"MODEL_NAME"];
        else if([specName isEqualToString:@"CPU_CORE"]) [returnDic setObject:coreCount  forKey:@"CPU_CORE"];
        else if([specName isEqualToString:@"MANUFACTURER"]) [returnDic setObject:production  forKey:@"MANUFACTURER"];
        else if([specName isEqualToString:@"RESOLUTION"]) [returnDic setObject:resolution  forKey:@"RESOLUTION"];
        else if([specName isEqualToString:@"ACCELEROMETER"]) [returnDic setObject:isAccelerometer  forKey:@"ACCELEROMETER"];
        else if([specName isEqualToString:@"GYROSCOPE"]) [returnDic setObject:isGyroscope  forKey:@"GYROSCOPE"];
        else if([specName isEqualToString:@"MAGNET"]) [returnDic setObject:isMagnetometer  forKey:@"MAGNET"];
        else if([specName isEqualToString:@"ORIENTATION"]) [returnDic setObject:isDirection  forKey:@"ORIENTATION"];
        else if([specName isEqualToString:@"PROXIMITY"]) [returnDic setObject:isProximity  forKey:@"PROXIMITY"];
        else if([specName isEqualToString:@"GPS"]) [returnDic setObject:isGPS  forKey:@"GPS"];
        else if([specName isEqualToString:@"STILL_CAM"]) [returnDic setObject:_isCamera  forKey:@"STILL_CAM"];
        else if([specName isEqualToString:@"FRONT_CAM"]) [returnDic setObject:isFrontCamera  forKey:@"FRONT_CAM"];
        else if([specName isEqualToString:@"OS_TYPE"]) [returnDic setObject:osName  forKey:@"OS_TYPE"];
        else if([specName isEqualToString:@"OS_VERSION"]) [returnDic setObject:osVersion  forKey:@"OS_VERSION"];
        else if([specName isEqualToString:@"APP_TYPE"]) [returnDic setObject:appType  forKey:@"APP_TYPE"];
        else if([specName isEqualToString:@"APP_VERSION"]) [returnDic setObject:appVersion  forKey:@"APP_VERSION"];
        else if([specName isEqualToString:@"DEVICE_ID"]) [returnDic setObject:dvcId  forKey:@"DEVICE_ID"];
        else if([specName isEqualToString:@"PHONE_NUM"]) [returnDic setObject:@"NONE"  forKey:@"PHONE_NUM"];
        ...
        [returnDic setObject:isProximity forKey:@"PROXIMITY"];
        
        [returnDic setObject:isGPS forKey:@"GPS"];
        [returnDic setObject:_isCamera forKey:@"STILL_CAM"];
        [returnDic setObject:isFrontCamera forKey:@"FRONT_CAM"];
        ...
        [returnDic setObject:@"NONE" forKey:@"PHONE_NUM"];
    }
...
-(void)getGpsLocation:(NSString *)callBackFunc :(NSString *)userSpecific {
    ...
    gpsCallback = callBackFunc;

    NSString *latitude = [NSString stringWithFormat:@"%f",self.locationManager.location.coordinate.latitude];
    NSString *longitude = [NSString stringWithFormat:@"%f",self.locationManager.location.coordinate.longitude];
    NSString *jsCommand;
    if (userSpecific == nil) {
        jsCommand = [NSString stringWithFormat:@"%@('SUCCEED', '%@', '%@');",callBackFunc,[latitude stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[longitude stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    }else{
        jsCommand = [NSString stringWithFormat:@"%@('%@','SUCCEED','%@', '%@');",callBackFunc,[userSpecific stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[latitude stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[longitude stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    }
    [self.locationManager stopUpdatingLocation];
//    [mywebView stringByEvaluatingJavaScriptFromString:jsCommand];
    [self evaluateJavaScript:jsCommand];
...
-(void)getMenuLocation:(NSString *)callbackFunc :(NSString *)userSpecific{
    ...
    if(resultDic.count>0){
        ...
        jsonString = @"이 기능은 웹앱에서만 사용할 수 있습니다.";
    }
...
//    NSString *jsCommand = [NSString stringWithFormat:@"%@('%@','%@');",_callbackFunc,[_userSpecific URLEncode],device.proximityMonitoringEnabled?@"YES":@"NO"];
    NSString *jsCommand = [NSString stringWithFormat:@"%@('%@','%@');",_callbackFunc,[_userSpecific URLEncode],device.proximityState?@"1":@"0"];
    [self evaluateJavaScript:jsCommand];
}
...
-(void) executeWebHistoryClear:(NSString *)callBackFunc :(NSString *)userSpecific{
    NSMutableDictionary *returnDic = [[NSMutableDictionary alloc]init];
...
//-(void)setRemoveWebViewData:(NSString *)callBackFunc :(NSString *)userSpecific :(BOOL)cookie :(BOOL)cache{
//    NSMutableArray *dataArr = [[NSMutableArray alloc] init];
//    if(cookie){
//        [dataArr addObject:WKWebsiteDataTypeCookies];
//    }
//
//    if(cache){
//        [dataArr addObject:WKWebsiteDataTypeDiskCache];
//        [dataArr addObject:WKWebsiteDataTypeMemoryCache];
//        [dataArr addObject:WKWebsiteDataTypeOfflineWebApplicationCache];
//        if (@available(iOS 11.3, *)) [dataArr addObject:WKWebsiteDataTypeFetchCache];
//    }
//
//    NSSet* nSet= [NSSet setWithArray:dataArr];
...
//        notice.body = [self.callBackDic objectForKey:@"message"];
//        AudioServicesPlaySystemSound(1007);
//        AudioServicesPlayAlertSound(kSystemSoundID_Vibrate);
//
//        UNNotificationRequest *request = [UNNotificationRequest requestWithIdentifier:@"mFinity" content:notice trigger:nil];
...
//        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:[encString dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:&error];
        ...
        NSString *jsCommand = [NSString stringWithFormat:@"%@('%@','%@');", [self.callBackDic objectForKey:@"callBackFunc"], [self.callBackDic objectForKey:@"userSpecific"], encString];
        [self evaluateJavaScript:jsCommand];
...
-(void) savePicture:(UIImage *)sendImage :(NSString*)file{
    ...
    if ([uploadFormat isEqualToString:[@"png" lowercaseString]]) {
        ...
        if(uploadFormat==nil) uploadFormat = @"jpeg";
...
//        [gpsTimer invalidate];
        ...
        NSDictionary *resultDic = [[NSDictionary alloc]initWithObjectsAndKeys:@"FAILED",@"RESULT", @"0",@"LATITUDE", @"0",@"LONGITUDE", @"0.0",@"ALTITUDE", @"0",@"TIME", @"0.0",@"ACCURACY", @"0.0",@"BEARING", @"0.0",@"SPEED", nil];
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:resultDic options:NSJSONWritingPrettyPrinted error:&error];
        NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
//        jsonString = [jsonString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        jsonString = [jsonString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];

        NSString *jsCommand = [NSString stringWithFormat:@"%@('%@','%@')",gisCallback,gisUserspec,jsonString];
//        [mywebView stringByEvaluatingJavaScriptFromString:jsCommand];
        [self evaluateJavaScript:jsCommand];

        isGisGps = NO;
    } else {
        NSString *jsCommand = [NSString stringWithFormat:@"%@('FAILED', '0.0', '0.0');",gpsCallback];
        [self.locationManager stopUpdatingLocation];
        [self evaluateJavaScript:jsCommand];
    }
}
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations{
    NSString *latitude = [NSString stringWithFormat:@"%f",locations.lastObject.coordinate.latitude];
    NSString *longitude = [NSString stringWithFormat:@"%f",locations.lastObject.coordinate.longitude];

    NSString *altitude = [NSString stringWithFormat:@"%f",locations.lastObject.altitude];
    
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"HH:mm:ss"];
    NSString *timestamp = [dateFormatter stringFromDate:locations.lastObject.timestamp];
    
    NSString *speed = [NSString stringWithFormat:@"%f",locations.lastObject.speed];
    NSString *accuracy = [NSString stringWithFormat:@"%f",locations.lastObject.horizontalAccuracy];
    NSString *bearing = [NSString stringWithFormat:@"%f",locations.lastObject.course];
