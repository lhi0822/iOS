...
//    [[UIBarButtonItem appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName:[appDelegate myRGBfromHex:appDelegate.naviFontColor]} forState:UIControlStateNormal];
    [[UIBarButtonItem appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName:[appDelegate myRGBfromHex:appDelegate.cNaviFontColor]} forState:UIControlStateNormal];
    
    imageView.backgroundColor = [appDelegate myRGBfromHex:appDelegate.cBgColor];
    myTableView.rowHeight = 50;
    
    [pageControl setTintColor:[UIColor lightGrayColor]];
    [pageControl setCurrentPageIndicatorTintColor:[appDelegate myRGBfromHex:appDelegate.cNaviColor]];
}
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    NSLog(@"curPage : %ld",(long)pageControl.currentPage);
    ...
    if (!isDrawMenu) {
        ...
        if (appDelegate.isAES256) {
            ...
            if ([appDelegate.demo isEqualToString:@"DEMO"]) {
                ...
                paramStr = [[NSString alloc]initWithFormat:@"up_menu_no=%@&devOs=I&devTy=%@&cuser_no=%@&returnType=JSON&encType=AES256",appDelegate.root_menu_no,appDelegate.dvcGubn,appDelegate.user_no];
            }else{
                urlString = [[NSString alloc] initWithFormat:@"%@/ezMainMenu2",appDelegate.main_url];
                paramStr = [[NSString alloc]initWithFormat:@"usrNo=%@&rootMenuNo=%@&devOs=I&devTy=%@&returnType=JSON&encType=AES256",appDelegate.user_no, appDelegate.root_menu_no,appDelegate.dvcGubn];
            }
        }else{
            if ([appDelegate.demo isEqualToString:@"DEMO"]) {
                urlString = [[NSString alloc] initWithFormat:@"%@/ezPubMenu2",appDelegate.main_url];
                paramStr = [[NSString alloc]initWithFormat:@"up_menu_no=%@&devOs=I&devTy=%@&cuser_no=%@&returnType=JSON",appDelegate.root_menu_no,appDelegate.dvcGubn,appDelegate.user_no];
                
            }else{
                urlString = [[NSString alloc] initWithFormat:@"%@/ezMainMenu2",appDelegate.main_url];
                paramStr = [[NSString alloc]initWithFormat:@"usrNo=%@&rootMenuNo=%@&devOs=I&devTy=%@&returnType=JSON",appDelegate.user_no, appDelegate.root_menu_no,appDelegate.dvcGubn];
            }
        }
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration{
    ...
    NSLog(@"willAnimateRotation viewWidth : %f / viewHeight : %f", viewWidth, viewHeight);
    ...
    int ICON_HEIGHT = 70;
    //아이콘 그리기 시작좌표
    int ICON_START_HORIZONTAL = 70;
    int ICON_START_VERTICAL = 70;
    //메뉴이름 넓이
    ...
    if ((toInterfaceOrientation == UIInterfaceOrientationPortrait)||(toInterfaceOrientation == UIInterfaceOrientationPortraitUpsideDown) ){
        ...
        TITLE_INCRESE_HORIZONTAL = ICON_INCRESE_HORIZONTAL;
        ...
        for(int i=0; i<[menuArray count]; i++){
            ...
            UIButton *uiButton = (UIButton *)[self.view viewWithTag:MENU_ICON_TAG+i];
            UILabel *uiLabel = (UILabel *)[self.view viewWithTag:MENU_LABEL_TAG+i];
            
            if ( i > 0 && i % countForPage == 0) {
            ...
            [uiLabel sizeToFit];
            CGPoint p = CGPointMake(uiButton.center.x, uiLabel.center.y);
            [uiLabel setCenter:p];
        }
    
    } else{
        horizonSpace = (viewWidth - (ICON_START_HORIZONTAL*2) - (ICON_WIDTH*rowCount)) / (rowCount-1);
        verticalSpace = (viewHeight - (self.navigationController.navigationBar.frame.size.height+[UIApplication sharedApplication].statusBarFrame.size.height+self.tabBarController.tabBar.frame.size.height+pageControl.frame.size.height) - (ICON_START_HORIZONTAL*2) - ((ICON_HEIGHT+TITLE_HEIGHT)*colCount)) / (colCount-1);
        ...
        for(int i=0; i<[menuArray count]; i++){
            if (ICON_CURRENT_HORIZONTAL + ICON_INCRESE_HORIZONTAL - ICON_START_HORIZONTAL > viewWidth * ( curPage + 1 )) {   //768
                ICON_CURRENT_HORIZONTAL = ICON_START_HORIZONTAL + viewWidth * curPage;               //768
                ICON_CURRENT_VERTICAL += ICON_INCRESE_VERTICAL;
                TITLE_CURRENT_HORIZONTAL = TITLE_START_HORIZONTAL + viewWidth * curPage;             //768
                TITLE_CURRENT_VERTICAL += TITLE_INCRESE_VERTICAL;
            }
            UIButton *uiButton = (UIButton *)[self.view viewWithTag:MENU_ICON_TAG+i];
            UILabel *uiLabel = (UILabel *)[self.view viewWithTag:MENU_LABEL_TAG+i];
...
-(void) menuSetting {
    ...
    if([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad){
        ...
        ICON_START_VERTICAL = 70;
        TITLE_WIDTH = ICON_WIDTH+8;
        TITLE_HEIGHT = 50;
        ...
        ICON_WIDTH = 70;
        ICON_HEIGHT = 70;
        ICON_START_HORIZONTAL = 25;
        ICON_START_VERTICAL = 25;
        TITLE_WIDTH = ICON_WIDTH+8;
        TITLE_HEIGHT = 30;
    }
    ...
    if ((interfaceOrientation == UIDeviceOrientationLandscapeLeft)||(interfaceOrientation == UIDeviceOrientationLandscapeRight)) {
        NSLog(@"가로");
        horizonSpace = (viewWidth - (ICON_START_HORIZONTAL*2) - (ICON_WIDTH*(colCount+1))) / colCount;
        verticalSpace = (viewHeight - (self.navigationController.navigationBar.frame.size.height+[UIApplication sharedApplication].statusBarFrame.size.height+self.tabBarController.tabBar.frame.size.height+pageControl.frame.size.height) - (ICON_START_HORIZONTAL*2) - ((ICON_HEIGHT+TITLE_HEIGHT)*(rowCount-1))) / (rowCount-2);
    }else {
        NSLog(@"세로");
        horizonSpace = (viewWidth - (ICON_START_HORIZONTAL*2) - (ICON_WIDTH*colCount)) / (colCount-1);
        verticalSpace = (viewHeight - (self.navigationController.navigationBar.frame.size.height+[UIApplication sharedApplication].statusBarFrame.size.height+self.tabBarController.tabBar.frame.size.height+pageControl.frame.size.height) - (ICON_START_HORIZONTAL*2) - ((ICON_HEIGHT+TITLE_HEIGHT)*rowCount)) / (rowCount-1);
    ...
    for (int index = 0; index < [menuArray count]; index++) {
        ...
        if (([_menuType isEqualToString:@"P"]) || ([_menuType isEqualToString:@"M"])) {
            ...
            UIColor *color = [appDelegate myRGBfromHex:appDelegate.cFontColor]; //[appDelegate myRGBfromHex:appDelegate.mainFontColor];
...
-(void) buttonToIndex:(int)indexBtn {
    ...
    NSLog(@"탭바 : %@", [[menuArray objectAtIndex:indexBtn]objectForKey:@"V17"] );
...
- (void)menuHandler{
    ...
    if ([menuKind isEqualToString:@"M"]) {
		...
		if ([menuType isEqualToString:@"B1"]) {
            ...
            if (data==nil || [dic objectForKey:appDelegate.menu_no]==nil || ![[dic objectForKey:appDelegate.menu_no] isEqualToString:currentAppVersion]) {
