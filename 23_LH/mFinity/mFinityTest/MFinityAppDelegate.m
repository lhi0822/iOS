...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    //APNS 메시지 수신후 PushReceiver.m 파일에서 처리하도록 하는 샘플
    //PushReceiver.m 파일은 필요에 따라 변경하여 사용
    //APNS 메시지 수신 -> PushManager -> PushReceiver
...
//    [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
    ...
    if([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) self.dvcGubn = @"T";
    else self.dvcGubn = @"P";
    
    _cBgColor = @"EFEFEF";
    _cNaviColor = @"0B57A0";
    _cNaviFontColor = @"FFFFFF";
    _cFontColor = @"000000";
    ...
    UITabBar *uiTabBar = [[UITabBar alloc]init];
    uiTabBar.delegate = self;
    
//    self.tabBarController.viewControllers = @[viewController1, viewController2, viewController3, viewController4, viewController5];
    self.tabBarController.viewControllers = @[viewController1, viewController4, viewController5]; //LH커스텀
...
//    [[UITabBar appearance] setTintColor:[self myRGBfromHex:_tabFontColor]];
    ...
    [self.tabBarController.tabBar setTintColor:[self myRGBfromHex:self.cNaviColor]];
    [self.tabBarController.tabBar setUnselectedItemTintColor:[UIColor lightGrayColor]];
    ...
    controllers = [NSDictionary dictionaryWithObjectsAndKeys:mainView,@"1",settingView,@"2",exit,@"3",noticeView,@"4",myMenuView,@"5",webView1,@"6",webView2,@"7",webView3,@"8",webView4,@"9",webView5,@"10",webView6,@"11", nil];
    NSArray *icons = [[NSArray alloc]initWithObjects:
                      [self getScaledImage:[UIImage imageNamed:@"icon1.png"] scaledToMaxWidth:25],
                      [self getScaledImage:[UIImage imageNamed:@"icon4.png"] scaledToMaxWidth:25],
                      [self getScaledImage:[UIImage imageNamed:@"icon5.png"] scaledToMaxWidth:25],
    ...
    NSDictionary *tabDict1 = [[NSDictionary alloc] initWithObjectsAndKeys:@"1",@"ICON", @"Home",@"ICONTITLE", @"1",@"TAB", @"Home",@"TITLE", @"Reserved Functionality", @"URL", nil];
    NSDictionary *tabDict2 = [[NSDictionary alloc] initWithObjectsAndKeys:@"2",@"ICON", @"Settings",@"ICONTITLE", @"2",@"TAB", @"Settings",@"TITLE", @"Reserved Functionality", @"URL", nil];
    NSDictionary *tabDict3 = [[NSDictionary alloc] initWithObjectsAndKeys:@"3",@"ICON", @"Exit",@"ICONTITLE", @"3",@"TAB", @"Exit",@"TITLE", @"Reserved Functionality", @"URL", nil];
    [testDict setObject:tabDict1 forKey:@"1"];
    [testDict setObject:tabDict2 forKey:@"2"];
    [testDict setObject:tabDict3 forKey:@"3"];
    
//    for (int i=0; i<[dic count]; i++) {
    for (int i=0; i<[testDict count]; i++) {
//        NSDictionary *subDic = [dic objectForKey:[NSString stringWithFormat:@"%d",i+1]];
        NSDictionary *subDic = [testDict objectForKey:[NSString stringWithFormat:@"%d",i+1]];
    ...
    if ([_menu_title isEqualToString:@"Exit"]) {
        _pre_tabID = ([prefs integerForKey:@"startTabNumber"]+1)%3;
        _target_url = [urlArray objectAtIndex:([prefs integerForKey:@"startTabNumber"]+1)%3];
        _menu_title = [_titleArray objectAtIndex:([prefs integerForKey:@"startTabNumber"]+1)%3];
        self.tabBarController.selectedIndex = ([prefs integerForKey:@"startTabNumber"]+1)%3;
    }else{
...
-(void)tabBarController:(UITabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController{
    ...
    UINavigationController *setting = [controllers objectForKey:@"2"];
    UINavigationController *exit = [controllers objectForKey:@"3"];
    
    UINavigationController *mymenu = [controllers objectForKey:@"4"];
    UINavigationController *notice = [controllers objectForKey:@"5"];
    ...
    if (state==UIApplicationStateActive) {
        ...
        if (_isLogin) {
            ...
            if ([type isEqualToString:@"M1"]) {
                ...
                alertView = [[UIAlertView alloc]initWithTitle:@"Notice" message:[alert valueForKey:@"body"] delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"message51", @""), nil];
                alertView.tag = 102;
                ...
                NSRange searchRange = NSMakeRange(0, [msg length]);
                NSRange range;
                while ((range = [msg rangeOfString:@"[" options:0 range:searchRange]).location != NSNotFound) {
                    [results addObject:[NSValue valueWithRange:range]];
                    searchRange = NSMakeRange(NSMaxRange(range), [msg length] - NSMaxRange(range));
                }
                ...
                NSString *compNo = [[msg substringFromIndex:range1.location+1] substringToIndex:2];
                NSString *authCode = [[msg substringFromIndex:range2.location+1] substringToIndex:5];
