...
-(id)initWithFrame:(CGRect)frame normalImage:(UIImage*)passiveImage andPressedImage:(UIImage*)activeImage withScrollview:(UIScrollView*)scrView
{
    ...
    if (self)
    {
        ...
        _buttonView.contentMode = UIViewContentModeScaleAspectFit;
...
//hilee_custom
-(id)initWithFrame:(CGRect)frame normalImage:(UIImage*)passiveImage andPressedImage:(UIImage*)activeImage withScrollview:(UIScrollView*)scrView naviHeight:(float)height isTranslucent:(BOOL)translucent
{
    self = [super initWithFrame:frame];
    if (self)
    {
        ...
        if(translucent){
//            _buttonView = [[UIView alloc]initWithFrame:CGRectMake(frame.origin.x, frame.origin.y+[UIApplication sharedApplication].statusBarFrame.size.height+44, frame.size.width, frame.size.height)];
//            _menuTable = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/4, [UIApplication sharedApplication].statusBarFrame.size.height+44, 0.75*SCREEN_WIDTH,SCREEN_HEIGHT - (SCREEN_HEIGHT - CGRectGetMaxY(self.frame)) )];
            _buttonView = [[UIView alloc]initWithFrame:CGRectMake(frame.origin.x, frame.origin.y+[UIApplication sharedApplication].statusBarFrame.size.height+height, frame.size.width, frame.size.height)];
            _menuTable = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/4, [UIApplication sharedApplication].statusBarFrame.size.height+height, 0.75*SCREEN_WIDTH,SCREEN_HEIGHT - (SCREEN_HEIGHT - CGRectGetMaxY(self.frame)) )];
            
        } else {
            _menuTable = [[UITableView alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/4, 0, 0.75*SCREEN_WIDTH,SCREEN_HEIGHT - (SCREEN_HEIGHT - CGRectGetMaxY(self.frame)) )];
        }
        
        _buttonView.backgroundColor = [MFUtil myRGBfromHex:[[MFSingleton sharedInstance] mainThemeColor]]; //[MFUtil myRGBfromHex:@"0093D5"];
        //_buttonView.backgroundColor = [MFUtil myRGBfromHex:@"1B4C98"];
        _buttonView.layer.cornerRadius = _buttonView.frame.size.width/2;
        _buttonView.clipsToBounds = YES;
        _buttonView.contentMode = UIViewContentModeScaleAspectFit;
        _buttonView.userInteractionEnabled = YES;
...
-(void) setupButton {
    ...
    //*hilee_custom
    UIView *vsview = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
    vsview.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.7];
    ...
    _normalImageView = [[UIImageView alloc]initWithFrame:CGRectMake(_buttonView.frame.size.width/2-(25/2), _buttonView.frame.size.height/2-(25/2), 25, 25)];
    ...
    _pressedImageView = [[UIImageView alloc]initWithFrame:CGRectMake(_buttonView.frame.size.width/2-(25/2), _buttonView.frame.size.height/2-(25/2), 25, 25)];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //[MFUtil myRGBfromHex:[[MFSingleton sharedInstance] mainThemeColor]];
    UIImage *img = [UIImage imageNamed:[_imageArray objectAtIndex:indexPath.row]];
    if (@available(iOS 13.0, *)) {
        img = [img imageWithTintColor:[MFUtil myRGBfromHex:[[MFSingleton sharedInstance] mainThemeColor]] renderingMode:UIImageRenderingModeAlwaysOriginal];
    } else {
...
//    cell.imgView.image = [UIImage imageNamed:[_imageArray objectAtIndex:indexPath.row]];
    ...
    cell.titleTxtView.text = [_labelArray objectAtIndex:indexPath.row];
