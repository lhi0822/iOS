...
//        [conn start];
        [conn start:^{
            ...
            if([[appDelegate.appPrefs objectForKey:@"IS_FIRST_LOGIN"] isEqual:@"FIRST"]){
                ...
                NSString *paramString = [NSString stringWithFormat:@"usrId=%@&dvcId=%@",[appDelegate.appPrefs objectForKey:@"USERID"], [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
        ...
        NSString *senderID = [NSString stringWithFormat:@"USER.SNS.%@.%@.%@.%@.%@", [[MFSingleton sharedInstance] appType],[[MFSingleton sharedInstance] dvcType],compNo, userID, dvcID];//[MFUtil getMfpsId];
...
//        }
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(10.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            NSString *paramString = [NSString stringWithFormat:@"queueName=%@&dvcId=%@",senderID, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            ...
            if ([wsName isEqualToString:@"getUserQueueInfo"]) {
                @try{
                    ...
                    if(arr.count>0){
                        ...
                        //이전의 커넥션 닫는 웹서비스 호출
                        [self callWebService:@"forceCloseConnection" WithParameter:[NSString stringWithFormat:@"connectionName=%@&dvcId=%@",connJsonData,[appDelegate.appPrefs objectForKey:@"DVC_ID"]]];
                    }
...
- (void)callDeletePush:(NSString *)num userInfo:(NSString *)userNo type:(NSString *)type{
    ...
    @try {
        ...
        if([type isEqualToString:@"CHAT"]){
            if([[NSString stringWithFormat:@"%@", userNo] isEqualToString:[NSString stringWithFormat:@"%@", self.myUserNo]]){
                urlStr = [NSString stringWithFormat:@"http://%@:15672/api/bindings/snsHost/e/mfps.topic/q/%@/%@.CHAT.%@.%@",[[MFSingleton sharedInstance] rmq_host], mfpsId,[[MFSingleton sharedInstance] appType],[appDelegate.appPrefs objectForKey:@"COMP_NO"],num];
            }
        } else {
            urlStr = [NSString stringWithFormat:@"http://%@:15672/api/bindings/snsHost/e/mfps.topic/q/%@/%@.BOARD.%@.%@.%@",[[MFSingleton sharedInstance] rmq_host], mfpsId,[[MFSingleton sharedInstance] appType],type,[appDelegate.appPrefs objectForKey:@"COMP_NO"],num];
        }
