...
- (void)callJoinSns:(NSString *)snsNo{
    ...
    NSString *paramString = [NSString stringWithFormat:@"usrNo=%@&snsNo=%@&dvcId=%@", myUserNo, snsNo, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
    MFURLSession *session = [[MFURLSession alloc]initWithURL:url option:paramString];
    session.delegate = self;
    [session start];
}
...
- (void)callChatReadStatus {
    ...
    @try{
        ...
        if(![[NSString stringWithFormat:@"%@", firstChat] isEqualToString:@"-1"] && ![[NSString stringWithFormat:@"%@", lastChat] isEqualToString:@"-1"]){
            ...
            NSString *paramString = [NSString stringWithFormat:@"usrNo=%@&roomNo=%@&firstChatNo=%@&lastChatNo=%@&dvcId=%@", myUserNo, self.roomNo, firstChat, lastChat, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            MFURLSession *session = [[MFURLSession alloc]initWithURL:url option:paramString];
            session.delegate = self;
            [session start];
        } else {
