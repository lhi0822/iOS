...
- (void)viewDidLoad {
    ...
    appDelegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    urlString = [[MFSingleton sharedInstance] mainUrl];
...
-(void)callWebService:(NSString *)serviceName :(NSString *)param{
    ...
    if([serviceName isEqualToString:@"getUserSNSLists"]){
        paramString = [NSString stringWithFormat:@"compNo=%@&usrId=%@&snsKind=1&searchNm=%@&dvcId=%@",compNo, [appDelegate.appPrefs objectForKey:@"USERID"], param, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
        
    } else if([serviceName isEqualToString:@"getPostLists"]){
        paramString = [NSString stringWithFormat:@"stPostSeq=%@&usrNo=%@&searchNm=%@&dvcId=%@",self.lastPostNo, myUserNo, param, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
        ...
        paramString = [NSString stringWithFormat:@"usrNo=%@&snsNo=%@&dvcId=%@", myUserNo, param, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
        ...
        paramString = [NSString stringWithFormat:@"usrNo=%@&compNo=%@&snsNo=%@&mfpsId=%@&isJoin=false&dvcId=%@", myUserNo, compNo, param, mfpsId, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
    }
...
- (void)refreshCallGetPostList:(NSString *)text{
    ...
    NSString *paramString = [NSString stringWithFormat:@"stPostSeq=1&usrNo=%@&searchNm=%@&dvcId=%@", myUserNo, text, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
    if (self.snsNo!=nil) {
        paramString = [paramString stringByAppendingFormat:@"&snsNo=%@",self.snsNo];
    }
