...
- (instancetype)init
{
    ...
    if (self) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSLog(@"SyncChatInfo myUserNo : %@", myUserNo);
        });
...
//        session = [NSURLSession sessionWithConfiguration:configuration delegate:self delegateQueue:[NSOperationQueue mainQueue]];
        ...
        myUserNo = [prefs objectForKey:[NSString stringWithFormat:@"%@_CUSERNO",userId]];
...
//    NSString *localDbPath = [NSString stringWithFormat:@"%@.db", [documentsDir stringByAppendingPathComponent:@"hilee"]];
//    sqlite3 *db;
//    if(sqlite3_open([localDbPath UTF8String], &db)){
//        NSLog(@"열림?");
//    } else{
//        NSLog(@"닫힘");
//    }
