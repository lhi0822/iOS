...
- (void)setCellMessageDateHeight:(CGFloat)cellMessageDateHeight
{
    NSParameterAssert(cellMessageDateHeight >= 0.0f);
    _cellMessageDateHeight = [self jsq_correctedLabelHeightForHeight:cellMessageDateHeight];
}

- (void)setCellSystemLabelHeight:(CGFloat)cellSystemLabelHeight
{
    NSParameterAssert(cellSystemLabelHeight >= 0.0f);
    _cellMessageDateHeight = [self jsq_correctedLabelHeightForHeight:cellSystemLabelHeight];
}
...
- (BOOL)isEqual:(id)object
{
    ...
    if (self.representedElementCategory == UICollectionElementCategoryCell) {
        ...
        if (![layoutAttributes.messageBubbleFont isEqual:self.messageBubbleFont]
            ...
            || (int)layoutAttributes.cellMessageDateHeight != (int)self.cellMessageDateHeight
            || (int)layoutAttributes.cellSystemLabelHeight != (int)self.cellSystemLabelHeight) {
            return NO;
        }
    }
...
- (instancetype)copyWithZone:(NSZone *)zone
{
    ...
    copy.cellMessageDateHeight = self.cellMessageDateHeight;
    copy.cellSystemLabelHeight = self.cellSystemLabelHeight;
