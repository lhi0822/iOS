...
//        NSLog(@"scrollPosition2 : %f", scrollPosition);
        ...
        if([self.isEdit isEqualToString:@"COMMENT"]){
            //텍스트를 입력할때마다 데이터 변경.
//            NSLog(@"textView.attributedText : %@", textView.attributedText);
...
- (void)textViewDidChangeSelection:(MFTextView *)textView {
    ...
    NSLog(@"prevLen : %ld / textView.text.length : %ld", (long)prevLen, textView.text.length);
...
//            UIFont *dic = [textView.attributedText attribute:NSFontAttributeName atIndex:textRange.location-1 effectiveRange:&range]; //원래
            ...
            [textView.textStorage insertAttributedString:attrSpace atIndex:textView.text.length+1];
...
//                                                                 [textView.textStorage deleteCharactersInRange:NSMakeRange(textRange.location-range.length, range.length+1)];
                                                                 ...
                                                                 [textView setSelectedRange:NSMakeRange(range.location+1, 0)]; //원래
