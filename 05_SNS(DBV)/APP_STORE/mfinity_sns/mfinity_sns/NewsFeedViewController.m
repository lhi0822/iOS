...
- (void)refreshCallGetPostList{
    @try{
        ...
        NSString *paramString = [NSString stringWithFormat:@"stPostSeq=1&usrNo=%@&searchNm=""&dvcId=%@", myUserNo, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
        if (self.snsNo!=nil) {
            paramString = [paramString stringByAppendingFormat:@"&snsNo=%@",self.snsNo];
        }
...
//                    NSLog(@"contents : %@", contents);
                ...
                */
                ...
                if ([result isEqualToString:@"SUCCESS"]) {
                    ...
                    NSLog(@"self.normalDataArray : %@", self.normalDataArray);
                ...
                NSMutableArray *dataSets = [session.returnDictionary objectForKey:@"DATASET"];
                NSLog(@"getTaskList dataSets : %@", dataSets);
                NSString *seq = [[NSString alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(self.selectBoardKind==1){
        ...
        @try{
            ProjectFeedViewCell *projCell = (ProjectFeedViewCell *)[tableView dequeueReusableCellWithIdentifier:@"ProjectFeedViewCell"];
            if (projCell == nil) {
                NSArray *topLevelObject = [[NSBundle mainBundle] loadNibNamed:@"ProjectFeedViewCell" owner:self options:nil];
                
                for (id currentObject in topLevelObject) {
                    if ([currentObject isKindOfClass:[ProjectFeedViewCell class]]) {
                        projCell = (ProjectFeedViewCell *) currentObject;
                        [projCell setSelectionStyle:UITableViewCellSelectionStyleNone];
                    }
                }
            }
            [self setProjNewsFeedCell:projCell indexPath:indexPath];
...
-(void)setNewsFeedCell:(NewsFeedViewCell *)feedCell indexPath:(NSIndexPath *)indexPath{
    ...
    feedCell.contentImageView.tag = indexPath.row;
    ...
    //                NSLog(@"originImagePath : %@", originImagePath);
                    ...
                    if (originImagePath!=nil && ![originImagePath isEqualToString:@""]) {
                        ...
                        [feedCell.contentImageView sd_setImageWithURL:[NSURL URLWithString:originImagePath] placeholderImage:nil options:0 completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                            if(error!=nil) NSLog(@"setNewFeed Error : %@", error);
                            
//                            if(feedCell.cellIsLoad==NO){
//                                feedCell.cellIsLoad = YES;
//                                [self.tableView beginUpdates];
...
-(void)setProjNewsFeedCell:(ProjectFeedViewCell *)projCell indexPath:(NSIndexPath *)indexPath{
    @try{
        if(projCell!=nil && self.projectDataArray.count>0){
            NSDictionary *dataSetItem = [self.projectDataArray objectAtIndex:indexPath.item];
            NSLog(@"dataSetItem : %@", dataSetItem);
            NSString *profileImagePath = [NSString urlDecodeString:[dataSetItem objectForKey:@"STATUS_IMG"]]; //thumb
            NSString *snsName = [NSString urlDecodeString:[dataSetItem objectForKey:@"SNS_NM"]];
            NSString *taskDate = [NSString urlDecodeString:[dataSetItem objectForKey:@"TASK_DATE"]];
            NSString *writerName = [NSString urlDecodeString:[dataSetItem objectForKey:@"CUSER_NM"]];
            NSString *writerNo = [dataSetItem objectForKey:@"CUSER_NO"];
            NSString *taskTitle = [NSString urlDecodeString:[dataSetItem objectForKey:@"TASK_TITLE"]];
            NSString *taskStartDate = [NSString urlDecodeString:[dataSetItem objectForKey:@"TASK_START_DATE"]];
            NSString *taskEndDate = [NSString urlDecodeString:[dataSetItem objectForKey:@"TASK_END_DATE"]];
            NSNumber *taskStatus = [dataSetItem objectForKey:@"STATUS"];
            ...
            if (![profileImagePath isEqual:@""]) {
                UIImage *userImg = [MFUtil imageByScalingAndCroppingForSize:CGSizeMake(120, 120) :[MFUtil saveThumbImage:@"Profile" path:profileImagePath num:writerNo]];
                [projCell.userImgBtn setImage:userImg forState:UIControlStateNormal];

            } else{
                [projCell.userImgBtn setImage:[UIImage imageNamed:@"profile_default.png"] forState:UIControlStateNormal];
            }
            ...
            projCell.userName.text = writerName;
            projCell.writeDate.text = postDateString;
            projCell.teamName.text = snsName;
            
            [projCell.userImgBtn addTarget:self action:@selector(tapDetected:) forControlEvents:UIControlEventTouchUpInside];
            projCell.userImgBtn.tag = indexPath.item;
            
            projCell.projectIcon.image = [MFUtil getScaledImage:[UIImage imageNamed:@"project_schedule_blue.png"] scaledToMaxWidth:25.0f];
            projCell.projectTitle.text = taskTitle;
            ...
            if(taskStartDate.length<=0 && taskEndDate.length<=0){
                projCell.projectDate.text = @"미정";
            } else if(taskStartDate.length>0 && taskEndDate.length<=0){
                projCell.projectDate.text = [NSString stringWithFormat:@"%@ ~ 미정", sDateStr];
            } else if(taskStartDate.length<=0 && taskEndDate.length>0){
                projCell.projectDate.text = [NSString stringWithFormat:@"미정 ~ %@", eDateStr];
            } else {
                projCell.projectDate.text = [NSString stringWithFormat:@"%@ ~ %@", sDateStr, eDateStr];
            }
            
            
            [projCell.statusBtn setBackgroundColor:[UIColor clearColor]];
            [projCell.statusBtn setImage:[MFUtil getScaledImage:[UIImage imageNamed:@"project_progress.png"] scaledToMaxWidth:13.0f] forState:UIControlStateNormal];
            [projCell.statusBtn setImageEdgeInsets:UIEdgeInsetsMake(0.0, 0.0, 0.0, 0.0)];
            [projCell.statusBtn setTitleEdgeInsets:UIEdgeInsetsMake(0.0, 5.0, 0.0, 0.0)];
            [projCell.statusBtn setTitle:@"상태" forState:UIControlStateNormal];
            ...
            projCell.statusLbl.text = statusStr;
            
            [projCell.userBtn setBackgroundColor:[UIColor clearColor]];
            [projCell.userBtn setImage:[MFUtil getScaledImage:[UIImage imageNamed:@"project_member.png"] scaledToMaxWidth:13.0f] forState:UIControlStateNormal];
            [projCell.userBtn setImageEdgeInsets:UIEdgeInsetsMake(0.0, 0.0, 0.0, 0.0)];
            [projCell.userBtn setTitleEdgeInsets:UIEdgeInsetsMake(0.0, 5.0, 0.0, 0.0)];
            [projCell.userBtn setTitle:@"수행자" forState:UIControlStateNormal];
            projCell.userLbl.text = managerName;
            
            [projCell.proceedBtn setBackgroundColor:[UIColor clearColor]];
            [projCell.proceedBtn setImage:[MFUtil getScaledImage:[UIImage imageNamed:@"project_graph.png"] scaledToMaxWidth:13.0f] forState:UIControlStateNormal];
            [projCell.proceedBtn setImageEdgeInsets:UIEdgeInsetsMake(0.0, 0.0, 0.0, 0.0)];
            [projCell.proceedBtn setTitleEdgeInsets:UIEdgeInsetsMake(0.0, 5.0, 0.0, 0.0)];
            [projCell.proceedBtn setTitle:@"진행률" forState:UIControlStateNormal];
            
            //[cell.proceedBar setFrame:CGRectMake(cell.proceedBar.frame.origin.x, cell.proceedBar.frame.origin.y, cell.proceedBar.frame.size.width, 20)];
            [projCell.ProgressView setProgress:[taskProgress intValue]*0.01 animated:NO];
            
            projCell.commCnt.text = [NSString stringWithFormat:@"%@ %@",NSLocalizedString(@"comment", @"comment"), commCnt];
            projCell.viewCnt.text = [NSString stringWithFormat:@"%@",readCnt];
            
            //읽음카운트 20이상 줄바꿈 현상 수정
            NSDictionary *attributes = @{NSFontAttributeName: [projCell.viewCnt font]};
            CGSize textSize = [[projCell.viewCnt text] sizeWithAttributes:attributes];
            CGFloat strikeWidth = textSize.width;
