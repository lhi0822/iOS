...
- (void)callWebService:(NSString *)serviceName{
    ...
    @try{
        if([serviceName isEqualToString:@"getSNSMemberList"]){
            paramString = [NSString stringWithFormat:@"snsNo=%@&usrNo=%@&currentUserNos=&pSize=%d&stSeq=%@&dvcId=%@", self.snsNo, myUserNo, pSize, stSeq, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            ...
            paramString = [NSString stringWithFormat:@"mode=%@&compNo=%@&snsNo=%@&snsNm=%@&users=%@&usrNo=%@&snsKind=%@&dvcId=%@",mode, compNo, self.snsNo, self.snsName, jsonData, myUserNo, self.snsKind, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            
        } else if([serviceName isEqualToString:@"getSNSWaitingMemberList"]){
            paramString = [NSString stringWithFormat:@"snsNo=%@&pSize=%d&stSeq=%@&dvcId=%@", self.snsNo, pSize, stSeq, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            
        } else if([serviceName isEqualToString:@"getSNSInfo"]){
            paramString = [NSString stringWithFormat:@"usrNo=%@&snsNo=%@&snsKind=%@&dvcId=%@", myUserNo, self.snsNo, self.snsKind, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
        
        } else if([serviceName isEqualToString:@"rejectSNSMember"]){
            if(rejectJsonStr==nil||[rejectJsonStr isEqualToString:@""]){
                paramString = [NSString stringWithFormat:@"snsKind=%@&snsNo=%@&users=%@&usrNo=%@&dvcId=%@", self.snsKind, self.snsNo, rejectJsonStr, myUserNo, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            } else {
                paramString = [NSString stringWithFormat:@"snsKind=%@&snsNo=%@&users=%@&usrNo=%@&rejectMessage=%@&dvcId=%@", self.snsKind, self.snsNo, rejectJsonStr, myUserNo, rejectMsg, [appDelegate.appPrefs objectForKey:@"DVC_ID"]];
            }
        }
