...
- (void)login {
    ...
    dvcId = [appDelegate.appPrefs objectForKey:@"DVC_ID"];
    NSLog(@"login dvcid : %@", dvcId);
    ...
    if([legacyNm isEqualToString:@"ANYMATE"]){
        paramString = [paramString stringByAppendingString:[NSString stringWithFormat:@"&cpnCode=%@&compNo=%@&carrier=%@&extRam=%@&extTotVol=%@&extUseVol=%@&useVol=%@", cpnCode, compNo, carrier, extRam, extTotVol, extUseVol, useVol]];
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"noti_RmqConnect" object:nil userInfo:nil];
    ...
    NSLog(@"loginConnectServer 로그인 되었으니 MQ 연결!");
...
//            NSLog(@"dataSets : %@", dataSets);
            ...
            if([[dataSet objectForKey:@"RESULT_CODE"] intValue] == 1){
                ...
                [appDelegate.appPrefs setObject:[dataSet objectForKey:@"COMP_NO"] forKey:@"COM P_NO"];
                ...
                UIAlertAction* okButton = [UIAlertAction actionWithTitle:NSLocalizedString(@"done", @"done") style:UIAlertActionStyleDefault
                                                                 handler:^(UIAlertAction * action) {
                                                                     [appDelegate.appPrefs removeObjectForKey:@"DVC_ID"];
                ...
                //NOTCERT
                if([legacyNm isEqualToString:@"NONE"]||[legacyNm isEqualToString:@"ANYMATE"]){
