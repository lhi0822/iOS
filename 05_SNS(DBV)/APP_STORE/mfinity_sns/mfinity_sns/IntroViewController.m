...
- (void)viewDidLoad {
    ...
    NSString *cachePath = [NSString stringWithFormat:@"/%@/%@/%@/Cache", [NSHomeDirectory() stringByAppendingPathComponent:@"Documents"], bundleIdentifier, [prefs objectForKey:@"COMP_NO"]];
    [imgCache makeDiskCachePath:cachePath];
    
    NSLog(@"INTRO DVC_ID : %@", [prefs objectForKey:@"DVC_ID"]);
    dvcId = [prefs objectForKey:@"DVC_ID"];
...
//    NSLog(@"기존 : %@",[prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]]);
//    NSLog(@"현재 : %@",appDelegate.appDeviceToken);
    
    @try{
//        if(![[prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]] isEqualToString:appDelegate.appDeviceToken] || [prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]]==nil || [[prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]] isEqual:@"(null)"]){
        if(![[prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]] isEqualToString:appDelegate.fcmToken] || [prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]]==nil || [[prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]] isEqual:@"(null)"]){
            NSLog(@"푸시아이디 바뀜");

//            [prefs setObject:appDelegate.appDeviceToken forKey:[appDelegate setPreferencesKey:@"PUSHID1"]];
            [prefs setObject:appDelegate.fcmToken forKey:[appDelegate setPreferencesKey:@"PUSHID1"]];
            [prefs synchronize];
            ...
            NSString *mfpsId = [NSString stringWithFormat:@"USER.SNS.%@.%@.%@.%@.%@", [[MFSingleton sharedInstance] appType],[[MFSingleton sharedInstance] dvcType],[prefs objectForKey:@"COMP_NO"], [prefs objectForKey:@"USERID"], [prefs objectForKey:@"DVC_ID"]];
            ...
            [dlqDict setObject:[prefs objectForKey:@"COMP_NO"] forKey:@"COMP_NO"];
            ...
            [dlqDict setObject:[prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]] forKey:@"PUSH_ID"];
            [dlqDict setObject:[prefs objectForKey:@"USERID"] forKey:@"USER_ID"];
            [dlqDict setObject:[prefs objectForKey:@"DVC_ID"]forKey:@"DVC_ID"];
            [dlqDict setObject:mfpsId forKey:@"QUEUE_NAME"];
            
            NSLog(@"dlqDict : %@", dlqDict);
...
-(void)handleTimer:(NSTimer *)timer {
    ...
    userPwd = [prefs objectForKey:[appDelegate setPreferencesKey:@"USERPWD"]];
    legacyNm = [[MFSingleton sharedInstance] legacyName];
...
- (void)loginCheck{
    if([legacyNm isEqualToString:@"HHI"]){
        if([[MFSingleton sharedInstance] autoLogin]){
            NSLog(@"AUTO_LOGIN_DATE : %@", [prefs objectForKey:[appDelegate setPreferencesKey:@"AUTOLOGINDATE"]]);
            ...
            if([prefs objectForKey:[appDelegate setPreferencesKey:@"AUTOLOGINDATE"]]==nil||[[prefs objectForKey:[appDelegate setPreferencesKey:@"AUTOLOGINDATE"]] isEqual:@""]||[[prefs objectForKey:[appDelegate setPreferencesKey:@"AUTOLOGINDATE"]] isEqual:@"(null)"]||[[prefs objectForKey:@"{@AUTO_LOGIN_DATE}"] isEqual:@""]){
                NSString *snsDate = [formatter stringFromDate:today];
                [prefs setObject:snsDate forKey:[appDelegate setPreferencesKey:@"AUTOLOGINDATE"]];
                [prefs synchronize];
            }
            
            //1.자동로그인 시작 날짜
            NSDate *startDate = [formatter dateFromString:[prefs objectForKey:[appDelegate setPreferencesKey:@"AUTOLOGINDATE"]]];
...
//                                                                                 exit(0);
        ...
        dvcId = [prefs objectForKey:@"DVC_ID"]; //처음설치하면 null_devid 형식일거란말이지
        NSLog(@"url : %@, cpncode : %@, dvcId : %@, userId : %@", url, cpnCode, dvcId, userId);
        
        if(url==nil || cpnCode==nil){
            [self performSegueWithIdentifier:@"URL_SETTING_PUSH" sender:self];
            
        } else if(url!=nil && cpnCode!=nil && dvcId!=nil && userId!=nil){
            [[NSNotificationCenter defaultCenter] postNotificationName:@"PushIDChangeNotification" object:nil];
    ...
    } else if([legacyNm isEqualToString:@"NONE"]){
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"PushIDChangeNotification" object:nil];
    }
}
-(void)enterWorkApp{
...
+ (void)nextPage {
    ...
    if ([prefs objectForKey:@"USERID"] != nil) {
        ...
        NSLog(@"LASTTABITEM : %@", [prefs objectForKey:[appDelegate setPreferencesKey:@"LASTTABITEM"]]);

        //TABARR를 가져와서 라스트아이템이 몇번째에 있는지 확인, 그 인덱스를 처음 시작할때 사용하면 됨
        NSArray *tabArr = [prefs objectForKey:[appDelegate setPreferencesKey:@"TABITEM"]];
        NSUInteger tabIdx = [tabArr indexOfObject:[prefs objectForKey:[appDelegate setPreferencesKey:@"LASTTABITEM"]]];
...
//        [self presentViewController:rootViewController animated:YES completion:nil];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if([prefs objectForKey:@"SHARE_INFO"]!=nil){
...
- (void)callWebService{
    ...
    pushId1 = [prefs objectForKey:[appDelegate setPreferencesKey:@"PUSHID1"]];
    ...
    NSString *compNo = [prefs objectForKey:@"COMP_NO"];
...
- (void)callGetProfile {
    NSString *urlString = [[MFSingleton sharedInstance] mainUrl];
    NSString *userNo = [prefs objectForKey:[appDelegate setPreferencesKey:@"CUSERNO"]];
    NSString *paramString = [NSString stringWithFormat:@"usrNo=%@&dvcId=%@", userNo, [prefs objectForKey:@"DVC_ID"]];
                ...
                if ([[session.returnDictionary objectForKey:@"RESULT"] isEqualToString:@"SUCCESS"]) {
                    ...
                    if ([[dataSet objectForKey:@"RESULT_CODE"] intValue] == 1) {
                        ...
                        if([appDelegate.singleOnlineFlag isEqualToString:@"Y"]){
                            NSString *urlStr = [NSString stringWithFormat:@"http://%@:15672/api/exchanges/snsHost/mfps.topic/publish", [[MFSingleton sharedInstance] rmq_host]];
                            ...
                            [payloadDict setObject:[prefs objectForKey:@"USERID"] forKey:@"USER_ID"];
                            [payloadDict setObject:[prefs objectForKey:@"DVC_ID"] forKey:@"DEVICE_ID"];
                            ...
                            [dlqDict setObject:[NSString stringWithFormat:@"USER.%@", [prefs objectForKey:@"USERID"]] forKey:@"routing_key"];
                        ...
                        //SUCCEED
                        [prefs setObject:[dataSet objectForKey:@"COMP_NM"] forKey:[appDelegate setPreferencesKey:@"COMPNM"]];
                        [prefs setObject:[dataSet objectForKey:@"COMP_NO"] forKey:@"COMP_NO"];
                        [prefs setObject:[dataSet objectForKey:@"CUSER_NO"] forKey:[appDelegate setPreferencesKey:@"CUSERNO"]];
                        [prefs setObject:[dataSet objectForKey:@"USER_NM"] forKey:[appDelegate setPreferencesKey:@"USERNM"]];
                        [prefs setObject:[dataSet objectForKey:@"DEPT_NM"] forKey:[appDelegate setPreferencesKey:@"DEPTNM"]];
                        [prefs setObject:[dataSet objectForKey:@"DEPT_NO"] forKey:[appDelegate setPreferencesKey:@"DEPTNO"]];
                        [prefs synchronize];
...
//                                deployURL = @"https://roms.dbvalley.com/mfinity/upload/deploy/60/15/mFinitySNS_1.6.0-001.plist";
...
- (void)introConnectServer :(NSDictionary *)dataSet {
    NSLog(@"introConnectServer 로그인 되었으니 MQ 연결! : %@", dataSet);
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        NSString *cuserNo = [dataSet objectForKey:@"CUSER_NO"];
//        NSLog(@"cuserNo : %@", cuserNo);
    ...
    [prefs setObject:appDelegate.bindQueueArr forKey:[appDelegate setPreferencesKey:@"BINDQARR"]];
    [prefs synchronize];
    
    NSDictionary *dic = [NSDictionary dictionaryWithObject:[prefs objectForKey:[appDelegate setPreferencesKey:@"BINDQARR"]] forKey:@"ROUTING_KEY"];
...
//    getUserSNSLists
    ...
    NSString *compNo = [prefs objectForKey:@"COMP_NO"];
    NSString *paramString = [NSString stringWithFormat:@"compNo=%@&usrId=%@&snsKind=%d&searchNm=""&dvcId=%@",compNo, [prefs objectForKey:@"USERID"], snsKind, [prefs objectForKey:@"DVC_ID"]];
//    NSString *paramString = [NSString stringWithFormat:@"compNo=%@&usrId=''&snsKind=%d&searchNm=""&dvcId=%@",compNo, snsKind, [prefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
