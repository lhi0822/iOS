...
- (void)_baseConfigs {
  ...
  self.segmentView = [[ARSegmentView alloc] init];
    //NSLog(@"segUserType : %@", self.segUserType);
    
    if([self.segUserType isEqualToString:@"1"]){
        [self.segmentView setTintColor:[UIColor whiteColor]]; //컨트롤 색상
        self.segmentView.bottomLine.backgroundColor = [UIColor whiteColor];
        
    } else {
        [self.segmentView setTintColor:[MFUtil myRGBfromHex:[[MFSingleton sharedInstance] mainThemeColor]]]; //컨트롤 색상
        self.segmentView.bottomLine.backgroundColor = [UIColor lightGrayColor];
    }
