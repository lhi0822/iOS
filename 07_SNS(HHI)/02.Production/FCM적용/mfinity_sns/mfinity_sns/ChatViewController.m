...
- (void)messagesInputToolbar:(JSQMessagesInputToolbar *)toolbar didPressRightBarButton:(UIButton *)sender {
   ...
   if([MFUtil isWorkingTime]) [self sendMessage:content];
}
...
-(void)receiveMyChatPush:(NSDictionary *)pushDict{
   @try{
      ...
      NSString *dvcId = [aditInfoDic objectForKey:@"DEVICE_ID"];
      ...
      if([[NSString stringWithFormat:@"%@", roomNo] isEqualToString:[NSString stringWithFormat:@"%@", self.roomNo]]){
         ...
         //여기
         NSString *myDvcId = [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]];
         NSLog(@"dvcid : %@ / myDvcId : %@", dvcId, myDvcId);
         if([dvcId isEqualToString:myDvcId]){
...
//                     isScroll = YES;
                     ...
                     NSLog(@"서버에서 가져온 제일 오래된 채팅 ; %@ / 서버에서 가져온 제일 최근 채팅 : %@", wsFirstChatNo, wsLastChatNo);
                     ...
                     if(firstChatInfo.count>0){
                        ...
                        if([[NSString stringWithFormat:@"%@", firstIsRead] isEqualToString:@"1"]){
                           ...
                           //ㄴ 다른기기에서 내가 보낸메시지일 경우 디비에 있고 isRead 1(읽음)인데 현재기기에서는 읽음 카운팅이 안될수도 있으니 호출해줘야함! (맞나?)
...
//         NSLog(@"SHARE MSG DATA : %@", [self.msgData.chatArray objectAtIndex:indexPath.row]);
      ...
      NSString *userNo = [[self.msgData.chatArray objectAtIndex:indexPath.row]objectForKey:@"USER_NO"];
      if([[NSString stringWithFormat:@"%@", userNo] isEqualToString:[NSString stringWithFormat:@"%@", _myUserNo]]){
         UIAlertAction *workAction1 = [UIAlertAction actionWithTitle:NSLocalizedString(@"(HSE)안전활동 등록", @"(HSE)안전활동 등록")
                                                               style:UIAlertActionStyleDefault
                                                             handler:^(UIAlertAction * action){
