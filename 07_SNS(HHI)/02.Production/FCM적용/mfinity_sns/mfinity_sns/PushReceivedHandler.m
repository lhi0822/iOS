...
- (void)callDeletePush:(NSString *)num userInfo:(NSString *)queueName type:(NSString *)type{
    ...
    @try {
        ...
        if([type isEqualToString:@"CHAT"]){
            urlStr = [NSString stringWithFormat:@"http://mfps2.hhi.co.kr:15672/api/bindings/snsHost/e/mfps.topic/q/%@/%@.CHAT.%@.%@",queueName,[[MFSingleton sharedInstance] appType],[appDelegate.appPrefs objectForKey:@"COMP_NO"],num];
        } else {
            urlStr = [NSString stringWithFormat:@"http://mfps2.hhi.co.kr:15672/api/bindings/snsHost/e/mfps.topic/q/%@/%@.BOARD.%@.%@.%@",queueName,[[MFSingleton sharedInstance] appType],type,[appDelegate.appPrefs objectForKey:@"COMP_NO"],num];
        }
