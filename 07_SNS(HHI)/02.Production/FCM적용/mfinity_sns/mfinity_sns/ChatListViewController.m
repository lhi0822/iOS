...
//    [self syncChatRoom]; //테스트버전배포
    
//    [self.tableView reloadData]; //사용안함
}
...
- (void)callDeleteChat: (NSIndexPath *)indexPath{
    @try{
        ...
        if(![[NSString stringWithFormat:@"%@", firstChat] isEqualToString:@"-1"] && ![[NSString stringWithFormat:@"%@", lastChat] isEqualToString:@"-1"]){
            ...
            NSString *paramString = [NSString stringWithFormat:@"usrNo=%@&roomNo=%@&firstChatNo=%@&lastChatNo=%@&dvcId=%@", self.myUserNo, self.deleteRoomNo, firstChat, lastChat, [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
            MFURLSession *session = [[MFURLSession alloc]initWithURL:url option:paramString];
            session.delegate = self;
            [session start];
        } else {
        ...
        if([roomType isEqualToString:@"2"]){
            ...
            NSString *paramString = [NSString stringWithFormat:@"roomNo=%@&usrNo=%@&queueName=%@&routingKey=%@&usrNm=%@&dvcId=%@",self.deleteRoomNo, userNo, mfpsId, routingKey, decodeUserNm, [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
...
//                        NSLog(@"currRoomArr : %@", currRoomArr);
                        ...
                        for(int i=0; i<dataSet.count; i++){
                            ...
                            formatter2.dateFormat = @"yyyy-MM-dd HH:mm:ss.0";
