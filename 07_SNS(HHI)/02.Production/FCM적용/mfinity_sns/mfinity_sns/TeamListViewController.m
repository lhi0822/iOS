...
- (void)callWebService:(NSString *)serviceName :(int)snsKind{
    ...
    if([serviceName isEqualToString:@"getUserSNSLists"]){
        paramString = [NSString stringWithFormat:@"compNo=%@&usrId=%@&snsKind=%d&searchNm=""&dvcId=%@",compNo, [appDelegate.appPrefs objectForKey:@"USERID"], snsKind, [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
        ...
        paramString = [NSString stringWithFormat:@"usrNo=%@&compNo=%@&snsNo=%@&mfpsId=%@&isJoin=false&dvcId=%@", myUserNo, compNo, cancelSNSNo, mfpsId,[appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
        cancelSNSNo = nil;
    }
