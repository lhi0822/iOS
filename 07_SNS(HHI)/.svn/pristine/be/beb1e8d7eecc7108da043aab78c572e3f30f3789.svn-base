    ...
    UIAlertAction *takePictureAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"popup_camera1", @"popup_camera1")
        ...
        if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            ...
            if([[MFSingleton sharedInstance] mediaAuthCheck] && [mediaAuth isEqualToString:@"0"]){
                ...
                [AccessAuthCheck cameraAccessCheck:^(BOOL status) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if(status==YES){
                            self.picker = [[UIImagePickerController alloc] init];
                            self.picker.delegate = self;
...
- (void)callWebService:(NSString *)serviceName{
    ...
    @try{
        if([serviceName isEqualToString:@"getSNSNo"]){
            paramString = [NSString stringWithFormat:@"usrNo=%@&compNo=%@&snsKind=%@&dvcId=%@", myUserNo, compNo, snsKind, [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
            ...
            paramString = [NSString stringWithFormat:@"usrNo=%@&targetUserNo=%@&snsNo=%@&snsNm=%@&snsTy=%@&snsNeedAllow=%@&snsDesc=%@&snsCoverImg=%@&compNo=%@&isCreateSNS=%@&mfpsId=%@&snsKind=%@&dvcId=%@", myUserNo, snsLeaderNo, self.snsNo, snsName, snsType, snsAllow, snsDesc, coverUrl, compNo, isCreateSns, mfpsId, snsKind, [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
        }
