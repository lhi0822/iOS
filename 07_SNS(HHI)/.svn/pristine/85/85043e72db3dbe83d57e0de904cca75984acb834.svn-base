...
-(void)callWebService:(NSString *)serviceName{
    @try{
        ...
        if([serviceName isEqualToString:@"withdrawSNS"]){
            ...
            paramString = [NSString stringWithFormat:@"usrNo=%@&compNo=%@&snsNo=%@&mfpsId=%@&isJoin=true&dvcId=%@", myUserNo, compNo, snsNo, mfpsId, [appDelegate.appPrefs objectForKey:[appDelegate setPreferencesKey:@"DVCID"]]];
