...
-(void) tileMenuSetting{
    ...
    if ([platformNumber isEqualToString:@"S"]) {
        platformNumber = @"5";
    }
    BOOL isPlusDevice;
    NSArray *arr = [platform componentsSeparatedByString:@" "];
    if ([[arr objectAtIndex:arr.count-1] isEqualToString:@"plus"]) {
        isPlusDevice = YES;
    }else{
        isPlusDevice = NO;
    }
    ...
    ICON_START_HORIZONTAL = 10;
    ICON_START_VERTICAL = 10;
    ICON_INCRESE_HORIZONTAL = 77; //75
    ICON_INCRESE_VERTICAL = 77; //105
...
-(void)tileMenuSetting2{
    ...
    int ICON_HEIGHT = 75;
    //아이콘 그리기 시작좌표
    int ICON_START_HORIZONTAL = 20;
    int ICON_START_VERTICAL = 20;
    //아이콘 그리기 증가좌표
    int ICON_INCRESE_HORIZONTAL = 0;
    int ICON_INCRESE_VERTICAL = 0;
    ...
    ICON_INCRESE_VERTICAL = ICON_HEIGHT + verticalSpace;
...
//    }
    ...
    if (colCount == 3) {
        ...
        /*
         if ([platform hasPrefix:@"iPhone 5"]||[platform hasPrefix:@"iPhone 6"]) {
         countForPage = 12;
         //iphone 5
         [mainScrollView setFrame:CGRectMake(mainScrollView.frame.origin.x, mainScrollView.frame.origin.y, mainScrollView.frame.size.width, mainScrollView.frame.size.height+100)];
         }else{
    ...
    for (int index = 0; index < [menuArray count]; index++) {
        ...
        if (([_menuType isEqualToString:@"P"]) || ([_menuType isEqualToString:@"M"])) {
            ...
            CGSize textSize = [[label text] sizeWithAttributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13]}];
...
- (void) coverFlowSetting {
    ...
    [coverflow setNumberOfCovers:(int)[menuArray count]];
    [SVProgressHUD dismiss];
    //isDrawMenu = YES;
...
-(void)pushNotificationReceived:(NSNotification *)notification{
    ...
    NSLog(@"pushNotificationReceived userInfo : %@",userInfo);
