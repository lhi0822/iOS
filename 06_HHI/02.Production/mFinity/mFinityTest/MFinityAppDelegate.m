...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [prefs removeObjectForKey:@"MDM_AUTH_TOKEN"];
    [prefs synchronize];
...
//    NSLog(@"[MDM] enterWorkApp urlString : %@",urlString);
...
//       }
    ...
    if ([sourceApplication isEqualToString:@"com.extrus.mdmclient"] && [url query]) {
        NSArray * param = [[url query] componentsSeparatedByString:@"&"];
        NSMutableArray *params = [NSMutableArray arrayWithArray:param];
        NSLog(@"11 param : %@",param);
        [params removeObject:@""];
        param = params;
        NSMutableString * resultMDMAgent = [NSMutableString string];
...
//        NSLog(@"UUID : %@", [MFinityAppDelegate getUUID]);
...
- (void)applicationWillEnterForeground:(UIApplication *)application{
    ...
    NSLog(@"%s", __func__);
    NSLog(@"mdmCallAPI : %@", self.mdmCallAPI);
...
- (void)setScrollTabBar:(NSDictionary *)dic{
    ...
    if (7>[osVersion intValue]>4) {
        ...
        NSLog(@"scrollTabDic.count : %lu", (unsigned long)scrollTabDic.count);
        float btnWidth = 0;
...
- (void)didTapClick:(UIButton *)sender{
    ...
    //NSLog(@"sender : %@", sender);
    //    NSLog(@"tabLineView.tag : %ld", (long)tabLineView.tag);
    
    //    UIView *tabLineView = [[UIView alloc] initWithFrame:CGRectMake(sender.frame.origin.x, sender.frame.size.height-5, sender.frame.size.width, 5)];
    //    tabLineView.backgroundColor = [UIColor redColor];
    //    [tabLineView setTag:sender.tag];
    //
    //    NSLog(@"tabLineView tag : %ld",(long)tabLineView.tag);
    //    NSLog(@"sender.frame.origin.x : %f", sender.frame.origin.x);
    ...
    if (state==UIApplicationStateActive) {
        ...
        if (_isLogin) {
            ...
            if ([type isEqualToString:@"M1"]) {
                ...
                alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message121", @"") message:contentsMessage delegate:self cancelButtonTitle:NSLocalizedString(@"message51", @"message51") otherButtonTitles:nil];
                alertView.tag = 101;
                ...
                alertView = [[UIAlertView alloc]initWithTitle:@"Message" message:body delegate:self cancelButtonTitle:nil otherButtonTitles:NSLocalizedString(@"message51", @""), nil];
                alertView.tag = 103;
                
            } else if([type isEqualToString:@"E"]){
                _userInfo = userInfo;
                if (_isLogin) {
                    alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message121", @"message121") message:body delegate:self cancelButtonTitle:NSLocalizedString(@"message52", @"message52") otherButtonTitles:NSLocalizedString(@"message51", @"message51"), nil];
                    alertView.tag = 104;
                }else{
                    _receivePush = YES;
            ...
            } else if([type isEqualToString:@"L1"]){
                NSString *msg = [userInfo objectForKey:@"msg"];
                NSError *error;
                NSDictionary *msgDict = [NSJSONSerialization JSONObjectWithData:[msg dataUsingEncoding:NSUTF8StringEncoding] options:kNilOptions error:&error];
                NSLog(@"msgDict : %@", msgDict);
                
                NSString *puserId = [msgDict objectForKey:@"USER_ID"];
                NSString *pdvcId = [msgDict objectForKey:@"DEVICE_ID"];
                NSString *puserNm = [msgDict objectForKey:@"USER_NM"];

                if([self.user_id isEqualToString:puserId]){
                    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
                    if(![[prefs objectForKey:@"DEVICE_ID"] isEqualToString:pdvcId]){
                        NSString *content = [NSString stringWithFormat:NSLocalizedString(@"one_dvc_only_msg", @"one_dvc_only_msg"), puserNm, puserId];
                        ...
                        [alert addAction:okButton];
                        //                     [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:alert animated:YES completion:nil];
                        UIViewController *top = [MFinityAppDelegate topViewController];
        ...
        if ([type isEqualToString:@"M2"]) {
            ...
            _userInfo = userInfo;
            if (_isLogin) {
                UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message121", @"message121") message:body delegate:self cancelButtonTitle:NSLocalizedString(@"message52", @"message52") otherButtonTitles:NSLocalizedString(@"message51", @"message51"), nil];
                alertView.tag = 104;
                [alertView show];
            }else{
                _receivePush = YES;
            ...
            if (_isLogin) {
              ...
              if([self.user_id isEqualToString:puserId]){
                 ...
                 if(![[prefs objectForKey:@"DEVICE_ID"] isEqualToString:pdvcId]){
                    NSLog(@"기기아이디 다름");
//                     "one_dvc_only_msg"="다른기기에서 %@계정이 접속되어 해당 기기의 접속을 종료합니다.";
                    NSString *content = [NSString stringWithFormat:NSLocalizedString(@"one_dvc_only_msg", @"one_dvc_only_msg"), puserNm, puserId];
                    UIAlertController *alert = [UIAlertController alertControllerWithTitle:content message:nil preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction* okButton = [UIAlertAction actionWithTitle:NSLocalizedString(@"message51", @"message51") style:UIAlertActionStyleDefault
                                                                  handler:^(UIAlertAction * action) {
...
//                     [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:alert animated:YES completion:nil];
        ...
        if([type isEqualToString:@"M1"]){
            if(_isLogin){
                ...
                UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"message121", @"") message:contentsMessage delegate:self cancelButtonTitle:NSLocalizedString(@"message51", @"message51") otherButtonTitles:nil];
                alertView.tag = 101;
...
+(NSString *)isPLteResult{
    NSString *pIp = [[MFinityAppDelegate getIPAddress] substringToIndex:5];
    NSString *isPLte;
    if([pIp isEqualToString:@"10.49"]||[pIp isEqualToString:@"10.50"]||[pIp isEqualToString:@"10.150.190.*"]){
        isPLte = @"true";
    } else {
        isPLte = @"false";
    }
