...
//    [[UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[UIToolbar.class]]setTintColor:[appDelegate myRGBfromHex:@"#ffffff"]]; //화살표만 바뀜
//    [[UIBarButtonItem appearance] setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[appDelegate myRGBfromHex:@"#ffffff"], NSForegroundColorAttributeName,nil] forState:UIControlStateNormal];
}
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"message52", @"취소") style:UIBarButtonItemStyleBordered target:self action:@selector(cancelButtonClick)];
...
- (void)startDownload{
    ...
    urlCon = [[NSURLConnection alloc] initWithRequest:urlRequest delegate:self];
    if (urlCon) {
...
- (void)errorDownload{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"message166", @"닫기") style:UIBarButtonItemStyleBordered target:self action:@selector(errorButtonClick)];
}
- (void)endDownload{
    _tableView.scrollEnabled = YES;
    
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"message166", @"닫기") style:UIBarButtonItemStyleBordered target:self action:@selector(leftButtonClick)];
}
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    NSString *commonFileName = [[prefs objectForKey:@"COMMON_DOWNLOAD"]lastPathComponent];
    if ([lastPath isEqualToString:commonFileName]) {
        ...
        if (dic==nil) {
            dic = [[NSMutableDictionary alloc]initWithObjectsAndKeys:[prefs objectForKey:@"RES_VER"],@"RES_VER",nil];
        }else{
            [dic setObject:[prefs objectForKey:@"RES_VER"] forKey:@"RES_VER"];
        }
