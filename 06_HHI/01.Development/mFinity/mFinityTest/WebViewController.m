...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if([appDelegate.mainType isEqualToString:@"1"]){
        ...
        if([appDelegate.tabBarType isEqualToString:@"B"]){
            [mywebView setFrame:CGRectMake(mywebView.frame.origin.x, mywebView.frame.origin.y, mywebView.frame.size.width, appDelegate.scrollView.frame.origin.y-self.navigationController.navigationBar.frame.size.height-[UIApplication sharedApplication].statusBarFrame.size.height)];
        } else if([appDelegate.tabBarType isEqualToString:@"T"]){
...
- (void)webView:(UIWebView *)sender runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(id *)frame {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"TouchOne" message:message delegate:nil cancelButtonTitle:@"확인" otherButtonTitles: nil];
    [alert show];
}
- (BOOL)webView:(UIWebView *)sender runJavaScriptConfirmPanelWithMessage:(NSString *)message initiatedByFrame:(id *)frame {
    clicked = NO;

    UIAlertView *confirmDiag = [[UIAlertView alloc] initWithTitle:@"TouchOne" message:message delegate:self cancelButtonTitle:NSLocalizedString(@"취소", @"취소") otherButtonTitles:NSLocalizedString(@"확인", @"확인"), nil];
    [confirmDiag show];
