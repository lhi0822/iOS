...
- (void)viewDidLoad
{
    ...
    if ([_urlString isEqualToString:@"MY_MENU_CALL"]) {
        ...
        decryptData = [[NSData dataWithContentsOfFile:appDelegate.subBgImagePath] AES256DecryptWithKey:appDelegate.AES256Key];
    ...
    imageView.image = [UIImage imageWithData:decryptData];
...
//                param = [[NSString alloc]initWithFormat:@"usrNo=%@&rootMenuNo=%@&devOs=I&devTy=P&returnType=JSON&encType=AES256",appDelegate.user_no, appDelegate.menu_no];
                param = [[NSString alloc]initWithFormat:@"usrNo=%@&rootMenuNo=%@&devOs=I&devTy=P&isPLte=%@&returnType=JSON&encType=AES256",appDelegate.user_no, appDelegate.menu_no, [MFinityAppDelegate isPLteResult]];
            }
        }
    }else{
        if ([appDelegate.demo isEqualToString:@"DEMO"]) {
...
//            WebViewController *vc = [[WebViewController alloc] init];
            ...
            NSString *htmlFilePath = [webAppFolder stringByAppendingPathComponent:appDelegate.target_url];
            ...
            BOOL isFile = [fileManager isReadableFileAtPath:htmlFilePath];
...
//                    WebViewController *vc = [[WebViewController alloc] init];
                ...
                NSLog(@"isFile : %@",isFile?@"YES":@"NO");
                NSLog(@"appDelegate.menu_no : %@",appDelegate.menu_no);
                NSLog(@"[dic objectForKey:appDelegate.menu_no] : %@",[dic objectForKey:appDelegate.menu_no]);
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    NSLog(@"MFTableViewController methodName : %@", methodName);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"MFTable select isTabBar : %@", [[menuArray objectAtIndex:indexBtn]objectForKey:@"V17"]);
