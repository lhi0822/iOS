...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *fileString = [[_uploadFilePathArray objectAtIndex:indexPath.row] lastPathComponent];
    label.text = fileString;
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    if (progressTag < _uploadUrlArray.count-1) {
        ...
        [self.delegate returnUploadDataWithDictionary:self.returnDic error:nil];
        /*
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    NSString *errorMSG = [NSString stringWithFormat:@"%@\n%@",error.localizedDescription,connection.currentRequest.URL.absoluteString];
    [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:NO];
    UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"FAIL",@"FAIL")
                                                       message:errorMSG
                                                      delegate:self
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Refresh" style:UIBarButtonItemStylePlain target:self action:@selector(rightButtonClick)];
    [self endUpload];
    [self.delegate returnUploadDataWithDictionary:nil error:error.localizedDescription];
}
#pragma mark
