...
//    [compNoField resignFirstResponder];
    ...
    NSString *param;
    urlString = [[NSString alloc] initWithFormat:@"%@/KDCert",appDelegate.main_url];
...
-(void)callPushCert{
    ...
    urlString = [[NSString alloc] initWithFormat:@"%@/getNewCertKey",appDelegate.main_url];
    if (appDelegate.isAES256) {
//        param = [[NSString alloc]initWithFormat:@"id=%@&compNo=10&push_1=%@&push_2=-&dvcOS=iOS&dvcid=%@&dvcgubn=P&returnType=JSON&encType=AES256",encodingID, appDelegate.appDeviceToken, dvcid];
        param = [[NSString alloc]initWithFormat:@"id=%@&compNo=10&push_1=%@&push_2=-&dvcOS=iOS&dvcid=%@&dvcgubn=P&returnType=JSON&encType=AES256",encodingID, appDelegate.fcmToken, dvcid];
    }else{
//        param = [[NSString alloc]initWithFormat:@"id=%@&compNo=10&push_1=%@&push_2=-&dvcOS=iOS&dvcid=%@&dvcgubn=P&returnType=JSON",encodingID, appDelegate.appDeviceToken, dvcid];
        param = [[NSString alloc]initWithFormat:@"id=%@&compNo=10&push_1=%@&push_2=-&dvcOS=iOS&dvcid=%@&dvcgubn=P&returnType=JSON",encodingID, appDelegate.fcmToken, dvcid];
    }
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    else if([methodName isEqualToString:@"KDCert"]){
//    else if([methodName isEqualToString:@"ezUserCert3"]){
    ...
    } else if([methodName isEqualToString:@"getNewCertKey"]){
        [SVProgressHUD dismiss];
        [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:FALSE];
...
-(NSString *)isValue:(NSString *)name{
	...
	int indexValue = (int)[deviceArray indexOfObject:name];
