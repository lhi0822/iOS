...
-(void)viewWillAppear:(BOOL)animated{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(callMFinitySNS:) name:@"callMFinitySNS" object:nil];
...
-(void) Login{
    ...
    if (!isButtonClick) {
        ...
        if (isOffline) {
            ...
            NSString *isRooting = [MFinityAppDelegate isRooted]?@"Y":@"N";
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if ([alertView.title isEqualToString:NSLocalizedString(@"message54", @"")]) {
        ...
        NSURLConnection *conn = [[NSURLConnection alloc]initWithRequest:request delegate:self];
        [conn start];
    }else if ([alertView.title isEqualToString:NSLocalizedString(@"mFinity계정", @"mFinity계정")]) {
...
//            url = [url stringByAppendingFormat:@"&DEVICE_ID=%@&USER_ID=%@&IS_ROOT=%@&PUSH_ID1=%@&PUSH_ID2=-",[MFinityAppDelegate getUUID],appDelegate.user_id,isRooted,appDelegate.appDeviceToken];
            url = [url stringByAppendingFormat:@"&DEVICE_ID=%@&USER_ID=%@&IS_ROOT=%@&PUSH_ID1=%@&PUSH_ID2=-",[MFinityAppDelegate getUUID],appDelegate.user_id,isRooted,appDelegate.fcmToken];
        }
...
-(void)callMFinitySNS:(NSNotification *)notis{
    NSLog(@"%s",__FUNCTION__);
    isRemoteCall = YES;
    appDelegate.isRemoteCall = YES;
    /*
