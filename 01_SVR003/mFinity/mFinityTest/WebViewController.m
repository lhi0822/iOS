...
//    NSString *userAgent = [mywebView stringByEvaluatingJavaScriptFromString:@"navigator.userAgent"];
//    userAgent = [NSString stringWithFormat:@"%@ mFinity", userAgent];
...
-(void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    if ([mfnpName isEqualToString:@"executeAccessCheck"]) {
        NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
        NSString *osType = [dic objectForKey:@"osType"];
        [self executeAccessCheck:callBackFunc :osType];
    }
    
    else if ([mfnpName isEqualToString:@"executeBackKeyEvent"]) {
...
//        [self executeFileUpload:fileType :json :upLoadPath];
        ...
        NSString *streamingUrl = [dic objectForKey:@"streamUrl"];
    ...
    else if([mfnpName isEqualToString:@"exeWindowClose"]){
        [self rightBtnClick];
    }
}
...
//                [self evaluateJavaScript:jsCommand];
                ...
                //
            }else if([host isEqualToString:@"executeMFSync"]){
                NSDictionary *dic = [self getParameters:[url query]];
                NSLog(@"dic : %@",dic);
                NSString *dbName = [dic objectForKey:@"dbName"];
                ...
                NSString *callBackFunc = [dic objectForKey:@"callbackFunc"];
                NSString *userSpecific = [dic objectForKey:@"userSpecific"];
                [self executeMFSync:callBackFunc :userSpecific :dbName :tableName];
            ...
            else if ([host isEqualToString:@"executeAccessCheck"]) {
                NSDictionary *dic = [self getParameters:[url query]];
                NSLog(@"executeAccessCheck dic : %@", dic);
...
//    [webView stringByEvaluatingJavaScriptFromString:@"window.open = function (open) { return function  (url, name, features) { window.location.href = url; return window; }; } (window.open);"];
    [mywebView stringByEvaluatingJavaScriptFromString:@"window.open = function (open) { return function  (url, name, features) { window.location.href = url; return window; }; } (window.open);"];
...
//    [webView stringByEvaluatingJavaScriptFromString:close];
    [mywebView stringByEvaluatingJavaScriptFromString:close];
        ...
        if (IS_OS_8_OR_LATER) {
            if ([_webView canGoBack]) {
                ...
                if (![self.webView isEqual:[self.webViews objectAtIndex:0]]) {
                    NSLog(@"before webview count : %d",[self.webViews count]);
                    WKWebView *webview = [self.webViews lastObject];
                    [webview removeFromSuperview];
                    [self.webViews removeLastObject];
                    NSLog(@"after webview count : %d",[self.webViews count]);
...
#pragma mark
#pragma mark MFNP
-(void) executeAccessCheck:(NSString *)callBackFunc :(NSString *)osType{
    NSLog(@"callBackFunc : %@ / osType : %@", callBackFunc, osType);
    
    NSString *isApp = isWebView?@"true":@"false";
    
    NSMutableDictionary *dic = [[NSMutableDictionary alloc]init];
    [dic setObject:osType forKey:@"OS"];
    [dic setObject:isApp forKey:@"IS_APP"];
    NSString *returnStr = [self getJsonStringByDictionary:dic];
    
    NSString *jsCommand = [NSString stringWithFormat:@"%@('%@');", callBackFunc, [returnStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    NSLog(@"jsCommand : %@", jsCommand);
    [mywebView stringByEvaluatingJavaScriptFromString:jsCommand];
...
- (void)executeMFSync:(NSString *)callbackFunc :(NSString *)userSpecific :(NSString *)dbName :(NSString *)tableName{
    ...
    if(appDelegate.isOffLine){
        ...
        [dic setObject:NSLocalizedString(@"message112", @"") forKey:@"ERR_MSG"];
        NSString *returnStr = [self getJsonStringByDictionary:dic];
        ...
        if (![fileManager isReadableFileAtPath:documentPath]) {
            ...
            query = [query stringByAppendingFormat:@"%@",tableName];
            NSArray *resultArray = [self selectQuery:documentPath :query];
            if([resultArray count]>0){
                ...
                NSString *urlString = [NSString stringWithFormat:@"%@//%@/mfsync/dbsync/upload",[paths objectAtIndex:0],[paths objectAtIndex:1]];
...
-(void)getGpsLocation:(NSString *)callBackFunc :(NSString *)userSpecific {
    ...
    if(latitude!=nil && longitude!=nil) result = @"SUCCEED";
    ...
    if (userSpecific == nil) {
        ...
        jsCommand = [NSString stringWithFormat:@"%@('%@', '%@', '%@', '%@');",callBackFunc,[userSpecific stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding], result, [latitude stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding],[longitude stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    }
...
//            [self evaluateJavaScript:jsCommand];
            ...
            if(readArray!=nil){
                self.isSync = YES;
                //[self fileUploads:readArray :uploadArray];
...
- (void)setMfnpMethod:(WKUserContentController *)userController{
    ...
    [userController addScriptMessageHandler:self name:@"executeAccessCheck"];
...
//    _webView.UIDelegate = self;
    ...
    if (_isTabBar) {
        ...
        NSLog(@"is not TabBar");
        self.navigationController.navigationBarHidden = YES; //TabBar OFF일 경우 네비게이션 숨김.
