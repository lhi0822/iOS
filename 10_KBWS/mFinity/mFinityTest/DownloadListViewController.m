...
- (void)errorDownload{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Close" style:UIBarButtonItemStyleBordered target:self action:@selector(errorButtonClick)];
}
- (void)endDownload{
    _tableView.scrollEnabled = YES;
...
-(void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response{
    ...
    if(statusCode == 404 || statusCode == 500){
        ...
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Refresh" style:UIBarButtonItemStylePlain target:self action:@selector(rightButtonClick)];
        [self errorDownload];
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"Error" message:[NSString stringWithFormat:@"%ld Error \n %@",(long)statusCode,NSLocalizedString(@"message12", @"")] delegate:self cancelButtonTitle:NSLocalizedString(@"message51", @"") otherButtonTitles: nil];
        [alertView show];
...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data{
    ...
    if ([totalFileSize intValue]==1) {
        percentView.text = [NSString stringWithFormat:@"%d kb",[percent intValue]/1024];
    }else{
